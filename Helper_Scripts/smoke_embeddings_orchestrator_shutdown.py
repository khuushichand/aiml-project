#!/usr/bin/env python3
"""
Smoke test: start the embeddings worker orchestrator and ensure it shuts down on signal.

This script:
- launches the orchestrator as a subprocess,
...
def _build_command(config_path: Optional[str], workers: Optional[int], python_bin: str) -> List[str]:
    """Build the orchestrator command line."""
    cmd = [
        python_bin,
        "-m",
        "tldw_Server_API.app.core.Embeddings.worker_orchestrator",
    ]
    if config_path:
...
def main() -> int:
    """Run the orchestrator shutdown smoke test."""
    parser = argparse.ArgumentParser(
        description="Smoke test orchestrator shutdown behavior",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--config", help="Orchestrator YAML config path", default=None)
    parser.add_argument("--workers", type=int, help="Override workers per pool", default=None)
    ...
    cmd = _build_command(args.config, args.workers, args.python)
    print(f"[smoke] starting orchestrator: {' '.join(cmd)}")
    ...
    print(f"[smoke] sending SIG{args.signal.upper()} to orchestrator")
    _send_signal(proc, args.signal)

    try:
        proc.wait(timeout=max(0.1, args.shutdown_timeout))
    except subprocess.TimeoutExpired:
        print("[smoke] shutdown timed out; killing orchestrator")
        proc.kill()
        proc.wait(timeout=5)
        return 2
    ...
    print("[smoke] orchestrator shut down cleanly")
    return 0
