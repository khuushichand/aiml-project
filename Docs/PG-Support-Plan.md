...
- **Integration Progress**: ~98% — MediaDatabase now provisions DocumentVersionIdentifiers through PostgreSQL migrations, keeps media/keyword FTS vectors in sync via backend helpers, and shares the sequence-resync flow with ChaChaNotes. ChaChaNotes (including PostgreSQL FTS rebuild/search and link/tag maintenance), the auth `UserDatabase`, and the analytics feedback services continue to run on the shared backend; newly added dual-backend regression tests exercise ingestion + RAG flows on both databases. Prompt Studio’s backend adapter now handles project/prompt CRUD, signature management, test case CRUD/search/statistics, test run recording, evaluation tracking, and optimization APIs through the shared abstraction, and the dual-backend API suite validates core project/prompt flows over PostgreSQL. Workflow persistence resolves a shared backend adapter with schema migrations plus backend-aware CRUD/step/event/artifact helpers, and runtime execution now routes through the shared backend; high-volume validation and rollout guidance captured below.

Recent Updates (2025-10-12)
- Stabilized two flaky AuthNZ refresh tests by isolating app state per test (module-level fixture sets TEST_MODE/TESTING and clears dependency overrides before app import).
- MediaDatabase: permanent delete now routes through backend helpers (backend-agnostic SQL + rowcount) and performs best-effort FTS cleanup; ready for PostgreSQL.
- Continued Media parity: minor FTS/LIKE and ordering cleanups; additional migrations/docversion utilities remain on the list.
- **Functional PostgreSQL Support**: Near-complete — Postgres schema creation runs cleanly for Media, ChaChaNotes, Analytics, the AuthNZ user store, Prompt Studio, and the Workflow database; CRUD/FTS paths execute via backend abstractions; the migration utility copies SQLite content/ChaCha/analytics/workflow data into Postgres, resyncs sequences, and now upgrades persisted deployments to DocumentVersionIdentifiers. DB_Manager factories prefer shared SQL backends. **Prompt Studio services exercise the backend adapter end-to-end, and workflows migrate via `migrate_workflows_sqlite_to_postgres` with stress validation.** Outstanding work centers on nightly stress automation, expanded Prompt Studio API coverage, and production observability polish.
...
## Workflow Rollout Guidance

### Migration tooling
- Use `python -m tldw_Server_API.app.core.DB_Management.migration_tools --workflows-sqlite Databases/workflows.db --pg-host <host> --pg-port <port> --pg-database <db> --pg-user <user> --pg-password <pass>` to copy workflow definitions, runs, steps, events, and artifacts into PostgreSQL. The CLI truncates existing rows, copies in batches (default 500), reseeds sequences, and emits a summary table comparing row counts per entity.
- Combine the workflow flag with `--content-sqlite`, `--chacha-sqlite`, and `--analytics-sqlite` to perform a single coordinated cutover.

### Pool sizing & configuration
- Default connection settings for mixed workloads: `pool_size=15`, `max_overflow=30`, `pool_timeout=30` seconds. Increase `pool_size` to 25–30 when running large optimization fleets or high-frequency evaluation runs.
- Expose `TLDW_WORKFLOW_DB_POOL_SIZE`, `TLDW_WORKFLOW_DB_MAX_OVERFLOW`, and `TLDW_WORKFLOW_DB_TIMEOUT` in deployment manifests to override per environment.

### Operational checks
- Before migration: capture snapshots of `workflow_runs`, `workflow_steps`, and `workflow_events` counts from SQLite for comparison.
- After migration: run `pytest -k workflows_postgres_migrations` and the dual-backend workflow smoke tests (`tests/Workflows/test_dual_backend_workflows.py`, `tests/Workflows/test_dual_backend_engine.py`).
- Verify queue health by polling `/api/v1/workflows/status`; confirm the heartbeat timestamp advances and that pending counts match expectations.
- Monitor PostgreSQL via `pg_stat_activity` and configure alerts for long-running transactions (>60s) or connection exhaustion (>= pool_size + max_overflow).
- Enable the opt-in stress sweep once per rollout: `TLDW_WORKFLOW_STRESS=1 pytest -v tldw_Server_API/tests/Workflows/test_workflow_stress.py`. Latest run (2025-10-08) executed 250 concurrent runs and 1,000 step events per backend with zero failures and <1.5s average latency.
- After going live, schedule a weekly lightweight verification that replays a handful of workflow runs against staging PostgreSQL and compares the artifact count/size to baseline.

### Documentation updates
- `Docs/Deployment/Postgres_Migration_Guide.md` now includes workflow-specific commands, validation steps, and rollback notes.
- `Remaining-DB-Fixup-1.md` tracks workflow rollout readiness and references the stress results above.
...
- 🔄 Integration with existing code IN PROGRESS (Media_DB_v2 backend parity ~97% with DocumentVersionIdentifiers migrations, sequence sync, and FTS maintenance all wired; AnalyticsDatabase refactored; ChaChaNotes backend adapters + PostgreSQL bootstrap/migrations + FTS/search/rebuild/link/tag helpers live; the auth `UserDatabase` fronts the backend abstraction with embedded schema fallbacks; RAG retrievers and analytics_system use backend helpers; Prompt Studio now runs through the shared backend across project/prompt/signature/test/evaluation/optimization flows with unit + dual-backend API coverage; Workflow persistence resolves a DatabaseBackend, applies schema migrations, and is exercised via dual-backend tests. Remaining work focuses on workflow engine/runtime integration, migration tooling for large installations, and expanded end-to-end coverage.)
