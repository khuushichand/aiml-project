...
- **Integration Progress**: ~97% — MediaDatabase now provisions DocumentVersionIdentifiers through PostgreSQL migrations, keeps media/keyword FTS vectors in sync via backend helpers, and shares the sequence-resync flow with ChaChaNotes. ChaChaNotes (including PostgreSQL FTS rebuild/search and link/tag maintenance), the auth `UserDatabase`, and the analytics feedback services continue to run on the shared backend; newly added dual-backend regression tests exercise ingestion + RAG flows on both databases. Prompt Studio’s backend adapter now handles project/prompt CRUD, signature management, test case CRUD/search/statistics, test run recording, evaluation tracking, and optimization APIs through the shared abstraction, and the new dual-backend API suite validates core project/prompt flows over PostgreSQL. Remaining work centers on the Workflow store, which still requires backend abstraction.
- **Functional PostgreSQL Support**: Near-complete — Postgres schema creation runs cleanly for Media, ChaChaNotes, Analytics, the AuthNZ user store, and the Prompt Studio backend adapter; CRUD/FTS paths execute via backend abstractions; the migration utility copies SQLite content/ChaCha/analytics data into Postgres, resyncs sequences, and now upgrades persisted deployments to DocumentVersionIdentifiers. DB_Manager factories prefer shared SQL backends. **Prompt Studio’s services now exercise the backend adapter end-to-end (including runtime jobs and API dependencies), while the Workflow database remains SQLite-only.** Outstanding work centers on large-scale migration validation, production rollout tooling, expanded Prompt Studio API coverage, and the Workflow abstraction.
- **Tests Written**: Unit, integration, and dual-backend coverage now include ChaChaNotes PostgreSQL FTS flows, backend smoke tests for Media/analytics, RAG database retrievers, the migration tooling, Postgres media search SQL assertions, the Prompt Studio API parity suites (`test_media_postgres_support.py`, `test_analytics_backend.py`, `test_claims_retriever.py`, `test_migration_tools.py`, `test_migration_cli_integration.py`, `test_dual_backend_end_to_end.py`, `tests/prompt_studio/integration/test_dual_backend_prompt_studio.py`, `.../test_projects_prompts_flows.py`, `.../test_import_csv_and_async_eval.py`), and the new Workflow dual-backend smoke test (`tests/Workflows/test_dual_backend_workflows.py`). Initial hybrid/vector parity now runs inside `test_dual_backend_end_to_end.py`, with broader vector matrix coverage still outstanding.
