...
**Goal:** Replace the legacy Hugging Face CLI naming with `hf`, refresh adjacent install guidance toward `pip install -U "huggingface_hub"`, and keep package/import references unchanged.
...
**Goal:** Prove that the current runtime guidance still points users at the legacy CLI naming.
**Success Criteria:** The targeted unit test fails before implementation because it expects `hf download`.
**Tests:** `source /Users/macbook-dev/Documents/GitHub/tldw_server2/.venv/bin/activate && python -m pytest tldw_Server_API/tests/Audio/test_qwen3_asr.py -k validate_model_path_missing_no_download -v`
**Status:** Complete
...
**Success Criteria:** All current legacy CLI references are updated, and nearby guidance prefers `pip install -U "huggingface_hub"` where CLI setup is being described.
**Tests:** `rg -n "huggingface[-_]cli" /Users/macbook-dev/Documents/GitHub/tldw_server2/.worktrees/hf-cli-migration`
**Status:** In Progress
...
**Tests:** `source /Users/macbook-dev/Documents/GitHub/tldw_server2/.venv/bin/activate && python -m pytest tldw_Server_API/tests/Audio/test_qwen3_asr.py -k validate_model_path_missing_no_download -v`; `rg -n "huggingface[-_]cli" /Users/macbook-dev/Documents/GitHub/tldw_server2/.worktrees/hf-cli-migration`; `source /Users/macbook-dev/Documents/GitHub/tldw_server2/.venv/bin/activate && python -m bandit -r tldw_Server_API/app/core/Ingestion_Media_Processing/Audio/Audio_Transcription_Qwen3ASR.py tldw_Server_API/tests/Audio/test_qwen3_asr.py -f json -o /tmp/bandit_hf_cli_migration.json`
**Status:** Not Started
...
Change the assertion in `test_validate_model_path_missing_no_download` so it expects `hf download` in the error string instead of the legacy CLI wording.

**Step 2: Run test to verify it fails**

Run: `source /Users/macbook-dev/Documents/GitHub/tldw_server2/.venv/bin/activate && python -m pytest tldw_Server_API/tests/Audio/test_qwen3_asr.py -k validate_model_path_missing_no_download -v`

Expected: FAIL because the runtime string still contains the legacy CLI wording.
...
- Replace the module docstring examples from the legacy CLI wording to `hf download`.
- Replace the two user-facing `BadRequestError` download hints to `hf download`.
...
- Replace legacy CLI prose references with `hf`.
- Replace legacy CLI download examples with `hf download`.
...
- Where a touched section discusses gated or private repos, add or preserve concise `hf auth login` guidance.
- Leave Python package/import names unchanged.

**Step 3: Review wording**

Verify each touched section still reads naturally and does not mix old and new CLI names.
...
Run: `rg -n "huggingface[-_]cli" /Users/macbook-dev/Documents/GitHub/tldw_server2/.worktrees/hf-cli-migration`

Expected: no matches.

**Step 3: Run Bandit on touched Python scope**

Run: `source /Users/macbook-dev/Documents/GitHub/tldw_server2/.venv/bin/activate && python -m bandit -r tldw_Server_API/app/core/Ingestion_Media_Processing/Audio/Audio_Transcription_Qwen3ASR.py tldw_Server_API/tests/Audio/test_qwen3_asr.py -f json -o /tmp/bandit_hf_cli_migration.json`
