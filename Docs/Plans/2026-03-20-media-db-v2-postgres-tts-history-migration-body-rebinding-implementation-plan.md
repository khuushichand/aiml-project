# Media DB V2 Postgres TTS-History Migration Body Rebinding Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rebind PostgreSQL migration body method `v20` onto a package-owned
...
TTS-history migration body, rebind the canonical class method in
`media_database_impl.py`, and convert the legacy `Media_DB_v2` method to a
live-module compat shell. Leave `migrations.py` and
`_ensure_postgres_tts_history(conn)` untouched.

**Tech Stack:** Python 3.11, pytest, PostgreSQL backend abstraction, Loguru

---

### Task 1: Add Canonical Ownership And Compat-Shell Delegation Regressions For `v20`
...
- canonical `MediaDatabase._postgres_migrate_to_v20` is no longer legacy-owned
- legacy `_LegacyMediaDatabase._postgres_migrate_to_v20(conn)` delegates through
  a package helper

Match the style established by the completed `v12`-`v18` tranches.
...
git commit -m "test: add postgres tts history migration regressions"
```

### Task 2: Add Package-Native TTS-History Migration Body Helper

**Files:**
- Create: `tldw_Server_API/app/core/DB_Management/media_db/schema/migration_bodies/postgres_tts_history.py`
...
- a protocol requiring `_ensure_postgres_tts_history(conn)`
- `run_postgres_migrate_to_v20(db, conn)` delegating to
...
git commit -m "refactor: add postgres tts history migration helper"
```

### Task 3: Rebind Canonical `v20` Method
...
git commit -m "refactor: rebind postgres tts history migration body"
```

### Task 4: Convert Legacy `v20` To A Live-Module Compat Shell
...
Update `Media_DB_v2.py` so `_postgres_migrate_to_v20(conn)` delegates through
`import_module(...)` to the new helper module.

Keep the method present as a compat shell. Do not change
`_ensure_postgres_tts_history(conn)` or `migrations.py`.
...
git commit -m "refactor: delegate postgres tts history migration"
```

### Task 5: Add Focused TTS-History Helper Behavior Check
...
Add one small focused test that proves the package helper for `v20` invokes
...
git commit -m "test: cover postgres tts history helper path"
```
