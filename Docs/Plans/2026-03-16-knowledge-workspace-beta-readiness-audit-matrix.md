...
- Current classification correction:
  - the present real-backend workspace suite covers boot health, shell interactions, live pasted-source intake, and live grounded chat/search; the separate live studio-output probe now counts report, compare-sources, timeline, and slides as live-covered, while chat-completion/study-aid outputs stay mock-only or env-blocked in this audit backend
