...
| `/workspace-playground` | Add-source ingestion via Paste tab | `apps/tldw-frontend/e2e/workflows/workspace-playground.real-backend.spec.ts`, `apps/packages/ui/src/components/Option/WorkspacePlayground/__tests__/AddSourceModal.stage1.ingestion.test.tsx`, `apps/packages/ui/src/components/Option/WorkspacePlayground/__tests__/AddSourceModal.stage2.intake.test.tsx`, backend worker tests | Live-covered | Real-backend Playwright now posts pasted text through the live Add Sources modal, proves `/api/v1/media/add` succeeds, asserts the inserted row is immediately usable, and cleans up the created media ID; component regressions now lock `media_type` on both paste and upload requests so the backend validation bug cannot silently return, while backend conflict-regression tests now prove transient `mark_media_as_processed()` races do not mislabel successful embeddings runs as failed | Candidate for beta gate |
| `/workspace-playground` | Add-source ingestion via URL tab | `apps/tldw-frontend/e2e/workflows/workspace-playground.real-backend.spec.ts`, `apps/tldw-frontend/e2e/workflows/workspace-playground.spec.ts`, component tests | Live-covered | Real-backend Playwright now posts a public fixture URL through the live `URL` tab, proves `/api/v1/media/add` succeeds, waits for vector readiness, asserts the workspace keyword tag lands on the created media item, and verifies the inserted source becomes selectable in the real sources pane; deterministic coverage still locks the explicit in-UI `Processing` state, and the same backend retry path now prevents transient completion-state conflicts from leaving successful URL ingests stuck in false error state | Keep as live audit proof; replace the external fixture with an owned stable URL if this becomes a release gate |
