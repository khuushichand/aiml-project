...
- Current classification correction:
  - the present real-backend workspace suite covers boot health, shell interactions, live pasted-source intake, and live grounded chat/search; the separate live studio-output probe now counts report, compare-sources, timeline, slides, mind map, and data table as live-covered, while quiz/flashcards remain env-blocked in this audit backend
