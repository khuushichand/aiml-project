...
| `/workspace-playground` | Studio slides output from workspace | `apps/tldw-frontend/e2e/workflows/workspace-playground.output-matrix.probe.spec.ts` | Missing | The workspace live probe repeatedly timed out on `Slides`, so the route does not yet have successful proof in the current audit environment | Revisit in a dedicated workspace live env; do not count separate presentation-studio coverage as workspace proof |
| `/workspace-playground` | Studio mind map and data-table outputs | `apps/packages/ui/src/components/Option/WorkspacePlayground/__tests__/StudioPane.stage2.test.tsx`, `apps/tldw-frontend/e2e/workflows/workspace-playground.spec.ts` | Mock-only | These outputs are chat-completion-only in `StudioPane`, and the current audit backend returns `Mock response from test mode` for `/api/v1/chat/completions`, so the live workspace probe intentionally excludes them | Need a non-test-mode live backend to promote them |
| `/workspace-playground` | Studio quiz and flashcards study aids | `apps/packages/ui/src/components/Option/WorkspacePlayground/__tests__/StudioPane.stage2.test.tsx` | Mock-only | The attempted live quiz probe failed with `Provider 'openai' requires an API key. (POST /api/v1/quizzes/generate)`, so current workspace route proof is blocked by provider credentials rather than UI wiring | Need live provider credentials or a local provider-backed quiz/flashcard path before promotion |
...
- `/workspace-playground` targeted live studio-output matrix command:
  ...
  - outcome: `1 passed`, `0 failed`, runtime about `2.8s`
...
- `/workspace-playground` real-backend outcome after repairs: `4 passed`, `0 failed`, runtime about `21.9s`
...
- Current four-spec audit rerun:
  - command: `bunx playwright test e2e/workflows/knowledge-qa.spec.ts e2e/workflows/workspace-playground.spec.ts e2e/workflows/workspace-playground.real-backend.spec.ts e2e/workflows/workspace-playground.output-matrix.probe.spec.ts --reporter=line --workers=1`
  - outcome: `38 passed`, `0 failed`, runtime about `1.3m`
...
- Current classification correction:
  - the present real-backend workspace suite covers boot health, shell interactions, live pasted-source intake, and live grounded chat/search; the separate live studio-output probe now counts only report, compare-sources, and timeline as live-covered, while slides remain missing and chat-completion/study-aid outputs stay mock-only or env-blocked in this audit backend
