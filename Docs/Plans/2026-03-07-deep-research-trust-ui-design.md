...
2. otherwise lazily fetch the trust artifacts on demand for the selected run
3. otherwise show a neutral empty state while the run is still before synthesis
...
- if no bundle data is loaded, show a `Load trust details` button
