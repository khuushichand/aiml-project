...
3. The endpoint uses lightweight helpers in `chat_service.py` and delegates to `chat_orchestrator.chat_api_call(...)`. Provider routing/validation comes from the adapter + capability registries. A minimal compatibility shim (`Chat_Functions.chat_api_call`) remains for legacy callers/tests; new code should import `chat_orchestrator` directly.
