...
Compatibility note:
- During the deprecation window, additive compatibility headers can appear when legacy compatibility inputs are detected:
  - `Deprecation: true`
  - `Sunset: <HTTP-date>`
  - `Link: </api/v1/media/process-emails>; rel="successor-version"`
