...
- [x] MCP unified diagnostics helper  
  ...
  - The Prometheus scrape endpoint (`/api/v1/mcp/metrics/prometheus`) continues to use `require_admin_unless_public` as a compatibility shim so deployments can opt into unauthenticated scraping via `MCP_PROMETHEUS_PUBLIC=1`; this helper no longer gates any new admin surfaces and is documented as legacy-only. HTTP behavior for the diagnostics endpoints is covered by `tldw_Server_API/tests/AuthNZ_Unit/test_mcp_admin_permissions_claims.py`, which asserts 401/403/200 semantics for principals with and without the `system.logs` permission.
