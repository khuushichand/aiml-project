...
└─────────────┘     └────────────────────┘     └────────────┬─────────────┘     └─────────┬──────────┘
                                      ▲                     │                           │
                                      │                     ▼                           ▼
                             Idempotency layer       Core Jobs leasing            Metrics & status
                                      │                     │                           │
...
- **Jobs Orchestration:** Core Jobs (`app/core/Jobs/manager.py`) handles leasing/retries; `prompt_studio/jobs_adapter.py` bridges job status to Prompt Studio.
