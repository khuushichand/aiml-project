...
- 2026-02-18: Completed terminology migration for user-facing watchlists navigation and table/filter labels (`Feeds`, `Monitors`, `Activity`, `Articles`, `Reports`) with E2E assertions in `apps/extension/tests/e2e/watchlists.spec.ts`.
- 2026-02-18: Removed remaining fallback wording drift (`job` -> `monitor`) in Jobs/Runs/Outputs user-visible copy paths and filter help text.
- 2026-02-18: Clarified advanced schedule and settings jargon by updating cron placeholder language and renaming cluster heading to `Related Topics (Claim Clusters)` while preserving explanatory help text.
