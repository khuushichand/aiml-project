...
- Code:
  ...
  - Replace local normalization with provider iterator output (`LLM_Calls/chat_calls.*iter_sse_lines_*`) and `normalize_provider_line` fallback for non-string chunks. Suppress provider `[DONE]`; call `stream.done()` once.
