...
## 1. Summary
Unify background job orchestration on the core Jobs subsystem (`tldw_Server_API/app/core/Jobs`) so Embeddings, Chatbooks, and Prompt Studio share a single job lifecycle, status model, and admin surface. Embeddings stages retain a Redis Streams transport for throughput, while Jobs remains the root status/billing source of truth.
...
- Legacy EmbeddingJobManager/queue schemas (removed): `tldw_Server_API/app/core/Embeddings/job_manager.py`, `tldw_Server_API/app/core/Embeddings/queue_schemas.py`
...
**Expected deletions**: Legacy EmbeddingJobManager, Prompt Studio job manager, Chatbooks job queue shim, media_embedding_jobs_db, duplicate status enums, and redundant admin endpoints (all removed in Phase 3). Redis Streams remains the transport for embeddings stages.
...
- Chatbooks and Prompt Studio workers use the Jobs worker SDK; Embeddings stages use Redis Streams with Jobs root status.
...
## 6. In Scope
- Core Jobs remains the single source of truth for status/billing across domains.
- New adapters to migrate Embeddings root jobs, Chatbooks, and Prompt Studio job creation to Jobs.
...
- Pipeline stages are represented as separate job types or queues; embeddings stages are transported via Redis Streams with Jobs root status.
...
### 8.3 Domain Adapters
- Embeddings: use Redis Streams for stage transport (chunking → embedding → storage) while keeping a Jobs root record for status/billing.
- Embeddings content-only (`content_embeddings`) stays single-stage for now; inline payloads do not run through the multi-stage pipeline.
...
- Mapping matrix doc: `Docs/Product/Job_System_Unification_Mapping_Matrix.md`.
...
- Domains define job types and queues; shared worker SDK handles polling and acknowledgments (Embeddings stages use Redis Streams workers).
...
- Workers across domains use `app/core/Jobs/worker_sdk.py` to acquire and acknowledge jobs (Embeddings stages use Redis Streams workers).
...
- Implement adapters for Embeddings (root Jobs + Redis stage transport), Chatbooks, and Prompt Studio that map their APIs to Jobs.
...
- Migrate embeddings workers to Redis Streams worker (minimal pipeline).
...
- Cutover sequence: stop legacy workers, drain or cancel legacy queues, enable Jobs-backed adapters, then start Redis (embeddings) and Jobs (chatbooks/prompt studio) workers.
- If regressions are detected, stop Redis embeddings workers and Jobs workers, then pause new enqueues; legacy backends are removed, so rollback requires restoring legacy code.
...
Proposed flags (env):
- `EMBEDDINGS_JOBS_BACKEND=jobs` (core Jobs is the system of record; legacy values are ignored).
...
In-flight handling:
- Freeze legacy enqueue first.
- Let legacy workers drain for a fixed window (recommend 24h); after that, cancel remaining legacy jobs with `reason="migration_cutover"` if any remain.
...
- Result: Jobs pipeline throughput/latency regressed relative to Redis Streams; embeddings stages remain on Redis Streams (see `Docs/Performance/Embeddings_Jobs_vs_Redis_Benchmark.md`).
...
## 14. Open Questions
- Do any embedding stages require a high-throughput transport that Jobs cannot provide today? - Yes; keep Redis Streams for stage transport (Section 12.1 benchmark).
