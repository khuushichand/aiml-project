...
- Current state: commercial and local providers are implemented in monolithic modules that mix request shaping, streaming, error mapping, and config handling.
  - Commercial: `tldw_Server_API/app/core/LLM_Calls/chat_calls.py:1`
  - Local: `tldw_Server_API/app/core/LLM_Calls/local_chat_calls.py:1`
- Problems observed:
...
- Code reduction: ≥30% fewer lines in `chat_calls.py` and `local_chat_calls.py` by removing branching.
...
- Extract adapters for all providers: OpenAI, Anthropic, Groq, OpenRouter, Google (Gemini), Mistral, HuggingFace, Qwen, DeepSeek, Cohere, Moonshot, Z.AI, Bedrock, plus a generic OpenAI-compatible adapter used by several custom/local servers (llama.cpp, Kobold, Oobabooga, TabbyAPI, vLLM, Ollama, Aphrodite, MLX, Custom OpenAI v1/v2, local-llm).
...
- Add embeddings adapters + registry for OpenAI/HuggingFace/Google/MLX and wire the endpoint behind `LLM_EMBEDDINGS_ADAPTERS_ENABLED=1`.
...
- Embeddings expansion beyond OpenAI/HuggingFace/Google/MLX (additional providers and advanced features).
...
- Implement `openai_adapter.py`; route legacy shims to the registry-backed adapter.
...
- Port Qwen, DeepSeek, HuggingFace, Cohere, Moonshot, Z.AI, Bedrock, and generic OpenAI-compatible adapter used by local/custom servers (llama.cpp, Kobold, Oobabooga, TabbyAPI, vLLM, Ollama, Aphrodite, MLX, Custom OpenAI v1/v2, local-llm).
- Route legacy handlers to adapters for all migrated providers.
- Remove provider-specific branching from `chat_calls.py` and `local_chat_calls.py`; leave compatibility wrappers that call the registry.
...
- **Stages 0–4 complete.** All 23 chat adapters and 4 embeddings adapters (OpenAI, HuggingFace, Google, MLX) are implemented and covered by tests.
- Adapter registry is the default routing path for all provider calls.
- Native httpx paths working for OpenAI/Anthropic/Groq/OpenRouter/Google/Mistral.
- 44+ adapter test files passing in CI; `llm-adapters-suites` and `llm-adapters-native-matrix` CI jobs are green.
- Stage 3 monolith cleanup is complete: `chat_calls.py` and `local_chat_calls.py` contain only thin compatibility wrappers (deleted Feb 2026) plus embeddings functions.
- Providers endpoint reports registry-backed capabilities; shape validated by unit test.

Latest Changes (Feb 2026)
- Deleted all `chat_with_*` wrapper functions and `_call_adapter` shims from `chat_calls.py` and `local_chat_calls.py`.
- Moved shared utilities (`_safe_cast`, `_resolve_openai_api_base`, `_parse_data_url_for_multimodal`) to `adapter_utils.py`.
- Extracted duplicated `_is_client_like_error()` heuristic in `chat_service.py` into a shared helper.
- Updated test files to import from adapter registry path instead of legacy wrappers.

Remaining Work
- **Feature flag sunset**: Flip `STREAMS_UNIFIED` default to `"1"` and remove legacy branches; flip `LLM_EMBEDDINGS_ADAPTERS_ENABLED` once all embeddings native HTTP flags are validated; then remove `LLM_EMBEDDINGS_NATIVE_HTTP_*` flags. See Feature Flag Sunset Plan below.
- **Circuit breaker unification**: Tracked in `IMPLEMENTATION_PLAN_circuit_breaker_unification_2026_02_08.md`. The ProviderManager's local CircuitBreaker class needs metrics integration.
- **Embeddings expansion**: Additional embeddings providers beyond OpenAI/HuggingFace/Google/MLX as needed.

## 11. Backward Compatibility
- Public FastAPI endpoints unchanged; request/response schema remains OpenAI-compatible.
- Legacy shims continue to exist, delegating to the registry, so orchestrators and tests remain intact.
...
- `record_success`/`record_failure` calls in `chat_service.py` are already complete for every terminal outcome.
- Remaining: ProviderManager's local `CircuitBreaker` class emits zero metrics; tracked in circuit breaker unification plan.
...
- Hidden coupling to legacy function signatures
  - Mitigation: keep wrappers and use adapter aliasing/compat helpers for argument translation during transition.
...
- Registry and base adapter modules exist; all in-scope chat adapters (OpenAI, Anthropic, Groq, OpenRouter, Google, Mistral, Qwen, DeepSeek, HuggingFace, Cohere, Moonshot, Z.AI, Bedrock, Custom OpenAI v1/v2, plus 8 local providers) are implemented and covered by tests.
...
- Embeddings adapters for OpenAI/HuggingFace/Google/MLX are wired behind `LLM_EMBEDDINGS_ADAPTERS_ENABLED=1` with endpoint parity tests.
...
- Docs: this PRD plus a short "Adding a new LLM adapter" guide in `Docs/Development/LLM_Adapters_Authoring_Guide.md`.

## 18. Deletions & Cleanup (after Phase 3)
- Remove provider-specific branching from `chat_calls.py` and `local_chat_calls.py`.
...
- Status (Feb 2026): **Complete.** All `chat_with_*` wrappers deleted. `local_chat_calls.py` deleted. Shared utilities relocated to `adapter_utils.py`. Only embeddings functions remain in `chat_calls.py` (~280 lines).

## 19. Open Questions (Resolved)
1. **Should embeddings be part of the same adapter registry or a sibling?** — **Decided**: sibling `EmbeddingsProviderRegistry` in `embeddings_adapter_registry.py`.
2. **Do we want provider-level retry policies configurable via registry?** — **Deferred**: adapters use http_client defaults; no per-provider override needed yet.
3. **Unify circuit breaker implementation across Chat/TTS/Evals?** — **In progress**: tracked in `IMPLEMENTATION_PLAN_circuit_breaker_unification_2026_02_08.md`.
4. **Any providers requiring non-HTTP transport (e.g., gRPC) in near term?** — **No**: no near-term need identified.
...
- [x] Authoring guide in `Docs/Development/LLM_Adapters_Authoring_Guide.md`
- [x] Import sanity check: registry import causes no cycles in API layers
- [x] CI green with no behavior changes

Verification
- [x] `python -m pytest -m "unit or integration" -q` passes
- [x] Lint/formatters (if configured) show no new warnings
...
- [x] Wire shim: make legacy handlers delegate to registry-backed adapter; preserve function signature
- [x] Tests
...
- [x] Implement Qwen, DeepSeek, HuggingFace, Cohere, Moonshot, Z.AI, Bedrock, generic OpenAI-compatible (for local/custom servers), MLX
...
- [x] Re-run entire LLM test suite including `tests/LLM_Calls/test_async_streaming_dedup.py` and strict filter tests
- [x] Delete `chat_with_*` wrapper functions from `chat_calls.py` (~800 lines) and `_call_adapter`/`_call_adapter_async` shims
- [x] Delete `local_chat_calls.py` entirely (663 lines, zero production callers)
- [x] Relocate shared utilities (`_safe_cast`, `_resolve_openai_api_base`, `_parse_data_url_for_multimodal`) to `adapter_utils.py`
- [x] Update test files to use `perform_chat_api_call(provider=...)` or direct adapter imports
...
- [x] Extend registry with HF/Google/MLX embeddings adapters: `providers/huggingface_embeddings_adapter.py`, `providers/google_embeddings_adapter.py`, `providers/mlx_embeddings_adapter.py`.
...
- [ ] Expand embeddings adapters to additional providers beyond OpenAI/HuggingFace/Google/MLX
...
- [x] Integrate `provider_manager.record_success/record_failure` in orchestrator paths that call adapters
- [ ] Ensure http_client metrics emit for adapter calls; add optional adapter-level counters (tracked in circuit breaker unification plan)
- [x] Keep prompt-safe logs using existing sanitization utilities (`_sanitize_payload_for_logging` used across all adapters)

Rollout & Safety
- [x] Adapter registry is the default routing path (no legacy feature flag).
- [x] Canary enable providers (OpenAI first) in non-prod, then prod
- [x] Rollback plan: deploy the previous release to restore legacy routing

Compatibility & Parity Checks
- [x] Streaming: exactly one final `[DONE]` from the endpoint (no duplicates) — verified across 4 dedup layers (adapter `seen_done`, `StreamingResponseHandler.done_sent`, `create_streaming_response_with_timeout` guard, `SSEStream.done_seen`)
- [x] Tool calling: identical behavior for `tool_choice` and `tools` presence
- [x] Error taxonomy: same HTTP status mapping at FastAPI layer — verified consistent across adapter `normalize_error()`, endpoint `ChatModuleException` handler, and `client_like_error` heuristic
- [x] Environment precedence for base URLs and keys matches legacy behavior

Definition of Done (Phase 1–4)
- [x] Registry and base adapter in place with docs
- [x] All in-scope chat adapters implemented and covered by tests
- [x] `/api/v1/chat/completions` streaming and non-streaming regression tests pass
- [x] Providers endpoint reports registry-backed capabilities
- [x] Embeddings adapters wired behind `LLM_EMBEDDINGS_ADAPTERS_ENABLED=1` with endpoint parity tests
- [x] Monolith branching removed; wrappers deleted; duplicated helpers relocated — `chat_calls.py` reduced to ~280 lines (embeddings only); `local_chat_calls.py` deleted

## 21. Feature Flag Sunset Plan

| Flag | Current Default | Sunset Condition | Action |
|------|----------------|-----------------|--------|
| `STREAMS_UNIFIED` | `"0"` in code, `"1"` in Docker compose | Rollout documented as "Complete" | Flip default to `"1"`, then remove flag + legacy branches in 5 endpoint files |
| `LLM_EMBEDDINGS_ADAPTERS_ENABLED` | `""` (off) | All 3 embeddings native HTTP flags validated | Flip to `"1"`, then remove flag |
| `LLM_EMBEDDINGS_NATIVE_HTTP_OPENAI` | off | OpenAI native HTTP validated in CI | Flip on, then delete legacy `get_openai_embeddings*` from `chat_calls.py` |
| `LLM_EMBEDDINGS_NATIVE_HTTP_HUGGINGFACE` | off | HF native HTTP validated | Flip on |
| `LLM_EMBEDDINGS_NATIVE_HTTP_GOOGLE` | off | Google native HTTP validated | Flip on |
...
- Shared Utilities: `tldw_Server_API/app/core/LLM_Calls/adapter_utils.py` (safe_cast, resolve_openai_api_base, etc.)
