...
- Preferences: user-editable settings (UI defaults, preferred models, prompt defaults) rendered with effective values (org/team/user/default) and optional source attribution.
...
- Catalog follows the privilege catalog pattern (YAML + versioning) and is owned by the backend API team; the profile response includes `catalog_version`.
- Proposed location: `tldw_Server_API/Config_Files/user_profile_catalog.yaml` with a loader/validator modeled on `tldw_Server_API/app/core/AuthNZ/privilege_catalog.py`.
...
- `include_raw`: admin-only, includes raw stored overrides (`raw_overrides`) grouped by scope.
...
- `PATCH /api/v1/admin/users/{user_id}/profile`
  - Admin updates any editable fields per catalog and scope; org/team editability rules still apply.
...
- `profile_version` reflects user record updates plus profile override tables; membership-only changes may not bump the version.
- Error responses (update endpoints):
  ...
  - `422 Unprocessable Entity`: request schema validation failure (invalid payload shape).
- Error payload follows standard API shape, e.g.:
    ...
    ```
  - Note: update requests are validated as a whole; invalid or forbidden keys return a structured error response and no changes are applied. `skipped` is reserved for bulk per-user results and unexpected runtime skips.
...
- Requires explicit `confirm=true` when the target count exceeds the default threshold of 1000 users.
  - Override via config.txt `[user_profile] bulkUpdateConfirmThreshold` (fallback sections: `profile`, `admin`) or env var `BULK_UPDATE_CONFIRM_THRESHOLD`.
...
```json
{
  ...
  "preferences": {
    "preferences.ui.theme": {"value": "paper", "source": "user"},
    "preferences.chat.default_model": {"value": "gpt-4.1-mini", "source": "org"}
  }
}
```

### Raw Overrides Shape (include_raw=true, admin-only)
```json
{
  "raw_overrides": {
    "user": [
...
- Admin profile access is **admin-role only**; scope enforcement still applies for target users (org/team membership + hierarchy); no cross-org leakage.
- Sensitive fields are always masked; secrets are never returned.
- Bulk updates require admin role.
...
- Batch/timeout guidance:
  ...
  - Default page size 25; max 100; validation errors return `422`.
- Request timeout target 10s for batch reads; clients should paginate with `page` + `page_size`.

## Deprecation Plan (v1)
- Endpoints to deprecate: `GET /api/v1/auth/me`, `GET /api/v1/users/me`, `PUT /api/v1/users/me`.
- Replacement: `GET /api/v1/users/me/profile` and `PATCH /api/v1/users/me/profile`.
- Timeline:
  - T0 (release): add new profile endpoints and start returning deprecation headers on legacy endpoints.
  - T0 + 2 releases (or 60 days): legacy endpoints remain functional but are marked deprecated in OpenAPI and docs.
  - T0 + 4 releases (or 120 days): legacy endpoints removed (or gated behind an explicit compatibility flag).
- Compatibility response (legacy endpoints during deprecation window):
  - Include headers: `Deprecation: true`, `Sunset: <RFC 1123 date>`, `Link: </api/v1/users/me/profile>; rel="successor-version"`.
  - Include warning payload shape in response body when feasible:
    ```json
    {"warning": "deprecated_endpoint", "successor": "/api/v1/users/me/profile"}
    ```
- OpenAPI deprecation notes:
  - Mark legacy operations with `deprecated: true` and include a description note pointing to the successor endpoint.
  - Add a short deprecation blurb to the schema examples for `/users/me` and `/auth/me`.
- Compatibility flag strategy:
  - Add a server config flag (e.g., `ENABLE_LEGACY_USER_ME_ENDPOINTS`) defaulting to true in the deprecation window.
  - When the flag is false, return `410 Gone` with a `successor` hint for legacy endpoints.
  - Default to false after the removal milestone; document any temporary overrides for rollback safety.
