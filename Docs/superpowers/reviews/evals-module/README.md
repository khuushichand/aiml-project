...
1. Severity: Medium
   ...
   Why it matters: The current batch orchestration route now passes `webhook_user_id=` into `evaluate_geval()`, `evaluate_rag()`, and `evaluate_response_quality()`, but the focused Slice 2 regression test still patches a service double that follows the older keyword contract. Instead of exercising strict fail-fast semantics, the route now aborts the whole batch with a 500 `TypeError` as soon as the patched service is invoked. This is a real orchestration contract regression in the dirty tree because the batch route’s callable shape no longer matches established route-level assumptions used by the focused test harness.
   ...
   Recommended fix: Re-stabilize the batch route/service contract in one place. Either update the route-facing test doubles and any other callers to accept the new optional `webhook_user_id` keyword, or insert a compatibility wrapper so batch orchestration only passes kwargs guaranteed by the older service interface when webhook identity is not required.
   Recommended tests: Keep `tldw_Server_API/tests/Evaluations/test_evaluations_stage3_batch_failfast_and_metrics_none.py` as the regression guard, but update it to assert the intended fail-fast behavior after the keyword contract is reconciled. Add a dedicated batch-route test that patches a minimal service double with the supported keyword set and verifies both the strict fail-fast cancellation path and the non-500 response contract.
   Verification note: `git show ec30354a2:tldw_Server_API/app/api/v1/endpoints/evaluations/evaluations_unified.py` shows the baseline batch route passed only `user_id=user_id` to these service helpers. The current dirty-tree route adds `webhook_user_id=batch_webhook_user_id`, while both the baseline and current service methods already accept `webhook_user_id`. The focused pytest run now fails with `TypeError: ... evaluate_geval() got an unexpected keyword argument 'webhook_user_id'`, confirming this is a working-tree-specific route/caller contract change rather than a baseline service bug.
### Open Questions
- Needs verification: there appears to be a startup race between `UnifiedEvaluationService.create_run()` and `cancel_run()`. `create_run()` detaches `_run_evaluation_async()` with `asyncio.create_task(...)` before any durable task handle is stored, while `_run_evaluation_async()` only registers `runner.running_tasks[run_id]` after the spawned task starts running, and `cancel_run()` consults only `runner.cancel_run(run_id)` before falling back to a DB-only status update. If a caller cancels immediately after `create_run()` returns, the DB can be marked `cancelled` even though the background task may not yet be registered or stopped. File references: `tldw_Server_API/app/core/Evaluations/unified_evaluation_service.py:493`, `tldw_Server_API/app/core/Evaluations/unified_evaluation_service.py:515`, `tldw_Server_API/app/core/Evaluations/unified_evaluation_service.py:619`, `tldw_Server_API/app/core/Evaluations/eval_runner.py:2363`. I did not record this as a normal finding because the current environment made clean end-to-end reproduction noisy; it should be validated with a targeted async regression test before promotion to Findings.
### Verification Run
- `git show ec30354a2:tldw_Server_API/app/core/Evaluations/eval_runner.py | sed -n '240,1405p'`
  ...
  Result: baseline verification for the runner/service orchestration paths. The baseline already uses detached background run startup, per-sample timeouts in `_process_batch()`, and service helper signatures that accept optional `webhook_user_id`.
