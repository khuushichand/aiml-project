...
- Streaming is limited to GGUF backbones and non-WAV output (PCM/MP3/OPUS); WAV streaming is rejected.
- A bundled default voice is registered on first use from `/Helper_Scripts/Audio/Sample_Voices/Sample_Voice_1.wav`.
...
- **Adapter**: Enforce GGUF-only streaming and non-WAV streaming output (PCM/MP3/OPUS).
- **Voice manager**:
...
- PCM s16le at 24kHz is the default streaming format; MP3/OPUS are also supported.
