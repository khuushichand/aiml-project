...
- Remaining inline SQL in AuthNZ touching users, sessions, and MFA (e.g., selected paths in `session_manager`, `mfa_service`, and parts of `initialize`/`monitoring`) is explicitly out-of-scope for this phase and will be migrated in later iterations as tests and PRDs for those areas are extended. A focused audit of `hasattr(conn, 'fetch*')` branches confirms the remaining clusters:
  - **Sessions**: core session creation, validation, refresh, listing, and cleanup now go through `AuthnzSessionsRepo`. The only remaining inline session SQL in AuthNZ lives in `token_blacklist.revoke_all_user_tokens`, which reads and updates `sessions` as part of “logout all devices” flows; this path is intentionally deferred for a later stage where it can be unified with `AuthnzSessionsRepo` without changing on-wire semantics.
  ...
  - **MFA**: `mfa_service.py` keeps dialect-conditional queries for MFA factors and verification attempts; a small `AuthnzMfaRepo` is planned to unify these paths in a follow-on iteration.
  - **API key manager / monitoring / initialize**: `api_key_manager.py`, `monitoring.py`, and parts of `initialize.py` use limited inline SQL for bootstrap and monitoring reads. These are currently low-risk and are explicitly marked as out-of-scope for Stage 3/4; they will be revisited once the core repos (users, sessions, token_blacklist, MFA) are fully in place.
