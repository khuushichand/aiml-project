...
- [ ] Bump the version in `apps/tldw-frontend/package.json` and verify it matches any release notes or tags.
- [ ] Update `apps/tldw-frontend/README.md` with user-visible changes and new setup steps.
- [ ] Ensure any new environment variables are documented in `apps/tldw-frontend/README.md` (and `.env.example` if present).
...
- [ ] Walk through the `apps/tldw-frontend/README.md` setup from a clean clone (install → dev → build).
- [ ] Verify screenshots, feature lists, and links are current.
- [ ] Update any feature docs or audit notes (for example `apps/tldw-frontend/FRONTEND_AUDIT.md`) to reflect major changes.
...
- [ ] Run `npm install` (or the project’s package manager) from `apps/tldw-frontend/`.
