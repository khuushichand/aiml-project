...
export class TldwModelsService {
  ...
  private transformModel(tldwModel: TldwModel): ModelInfo {
    ...
    const declaredType = (tldwModel.type || '').toLowerCase()
    if (declaredType) {
      if (declaredType === 'embedding') {
        type = 'embedding'
      } else if (declaredType !== 'chat') {
        type = 'other'
      }
      ...
      type = 'embedding'
    } else if (tldwModel.modalities) {
      const outputs = Array.isArray(tldwModel.modalities.output)
        ? tldwModel.modalities.output.map((v) => String(v).toLowerCase())
        : []
