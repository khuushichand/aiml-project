...
> Note: The shared UI source now lives under `packages/ui/src` (from the extension).
> The extension app is in `apps/extension`, and the web UI consumes the shared UI via `@tldw/ui`.
...
Likely add to `apps/tldw-frontend` (from extension)
...
- Tailwind v3 config in extension vs Tailwind v4 in `apps/tldw-frontend`.
