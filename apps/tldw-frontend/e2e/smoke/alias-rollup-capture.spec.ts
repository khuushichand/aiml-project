import fs from 'node:fs/promises';
import path from 'node:path';
import { test, expect } from '@playwright/test';
import { seedAuth } from './smoke.setup';

const TELEMETRY_KEY = 'tldw:route:alias:telemetry';
const LOAD_TIMEOUT = 30_000;

type AliasFlow = {
  from: string;
  expectedPath: string;
  expectedSearchIncludes?: string[];
};

type RollupRow = {
  path: string;
  hits: number;
  share: number;
};

type NormalizedRollupRow = RollupRow & {
  estimated_hits: number;
  estimated_share: number;
};

type TelemetryState = {
  counters?: Record<string, number>;
  alias_hits?: Record<string, number>;
  destination_hits?: Record<string, number>;
  last_event_at?: number | null;
  last_redirect?: {
    source_path: string;
    destination_path: string;
    preserve_params: boolean;
    query_or_hash_carried: boolean;
  } | null;
};

const CONTROLLED_ALIAS_FLOWS: AliasFlow[] = [
  { from: '/search?q=wk2-1', expectedPath: '/knowledge', expectedSearchIncludes: ['q=wk2-1'] },
  { from: '/search?q=wk2-2', expectedPath: '/knowledge', expectedSearchIncludes: ['q=wk2-2'] },
  { from: '/search?q=wk2-3', expectedPath: '/knowledge', expectedSearchIncludes: ['q=wk2-3'] },
  { from: '/config', expectedPath: '/settings' },
  { from: '/profile', expectedPath: '/settings' },
  { from: '/privileges', expectedPath: '/settings' },
  { from: '/audio', expectedPath: '/speech' },
  { from: '/reading', expectedPath: '/collections' },
  { from: '/claims-review', expectedPath: '/content-review' },
  { from: '/review', expectedPath: '/media-multi' },
  { from: '/admin', expectedPath: '/admin/server' },
  { from: '/admin/data-ops', expectedPath: '/admin/server' },
  { from: '/connectors/jobs', expectedPath: '/settings' },
  { from: '/media/123/view', expectedPath: '/media' },
];

const toRollupRows = (
  map: Record<string, number>,
  total: number,
  limit: number
): RollupRow[] => {
  if (!Number.isFinite(total) || total <= 0) return [];

  return Object.entries(map)
    .sort((a, b) => {
      if (b[1] !== a[1]) return b[1] - a[1];
      return a[0].localeCompare(b[0]);
    })
    .slice(0, limit)
    .map(([route, hits]) => ({
      path: route,
      hits,
      share: hits / total,
    }));
};

test('captures controlled week-2 alias telemetry rollup', async ({ page }, testInfo) => {
  await seedAuth(page);

  await page.goto('/', { waitUntil: 'domcontentloaded', timeout: LOAD_TIMEOUT });
  await page.waitForLoadState('networkidle', { timeout: LOAD_TIMEOUT }).catch(() => {});

  await page.evaluate((key) => {
    localStorage.removeItem(key);
  }, TELEMETRY_KEY);

  for (const flow of CONTROLLED_ALIAS_FLOWS) {
    await page.goto(flow.from, { waitUntil: 'domcontentloaded', timeout: LOAD_TIMEOUT });

    await page.waitForURL(
      (url) => {
        if (url.pathname !== flow.expectedPath) return false;

        const searchIncludes = flow.expectedSearchIncludes ?? [];
        return searchIncludes.every((token) => url.search.includes(token));
      },
      { timeout: LOAD_TIMEOUT }
    );

    await page.waitForLoadState('networkidle', { timeout: LOAD_TIMEOUT }).catch(() => {});
    await page.waitForTimeout(75);
  }

  const telemetryState = await page.evaluate((key) => {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    try {
      return JSON.parse(raw);
    } catch {
      return { parse_error: true, raw };
    }
  }, TELEMETRY_KEY);

  expect(telemetryState, 'Expected alias telemetry state in localStorage').toBeTruthy();
  expect((telemetryState as { parse_error?: boolean }).parse_error).not.toBe(true);

  const typedState = telemetryState as TelemetryState;
  const totalRedirects = typedState.counters?.route_alias_redirect ?? 0;
  const topAliasSources = toRollupRows(typedState.alias_hits ?? {}, totalRedirects, 10);
  const topDestinations = toRollupRows(typedState.destination_hits ?? {}, totalRedirects, 10);

  expect(totalRedirects).toBeGreaterThanOrEqual(CONTROLLED_ALIAS_FLOWS.length);

  const flowCount = CONTROLLED_ALIAS_FLOWS.length;
  const observedRedirectMultiplier =
    flowCount > 0 ? totalRedirects / flowCount : 1;
  const integralMultiplier =
    Number.isFinite(observedRedirectMultiplier) &&
    observedRedirectMultiplier >= 1 &&
    Math.abs(Math.round(observedRedirectMultiplier) - observedRedirectMultiplier) < 1e-9
      ? Math.round(observedRedirectMultiplier)
      : null;
  const normalizedTotalRedirects =
    integralMultiplier && integralMultiplier > 0
      ? Math.floor(totalRedirects / integralMultiplier)
      : totalRedirects;
  const toNormalizedRows = (rows: RollupRow[]): NormalizedRollupRow[] =>
    rows.map((row) => {
      const estimatedHits =
        integralMultiplier && integralMultiplier > 0
          ? Math.floor(row.hits / integralMultiplier)
          : row.hits;
      return {
        ...row,
        estimated_hits: estimatedHits,
        estimated_share:
          normalizedTotalRedirects > 0 ? estimatedHits / normalizedTotalRedirects : 0,
      };
    });

  const capture = {
    captured_at: new Date().toISOString(),
    source: 'controlled-alias-flows',
    run_context: {
      flow_count: flowCount,
      base_url: testInfo.project.use.baseURL,
      browser: testInfo.project.name,
      observed_redirect_multiplier: observedRedirectMultiplier,
      integral_multiplier: integralMultiplier,
    },
    rollup: {
      total_redirects: totalRedirects,
      last_event_at: typedState.last_event_at ?? null,
      top_alias_sources: topAliasSources,
      top_destinations: topDestinations,
      normalized_estimate: {
        total_redirects: normalizedTotalRedirects,
        top_alias_sources: toNormalizedRows(topAliasSources),
        top_destinations: toNormalizedRows(topDestinations),
      },
      last_redirect: typedState.last_redirect ?? null,
    },
    flows: CONTROLLED_ALIAS_FLOWS,
  };

  const outputPath = path.resolve(
    process.cwd(),
    '../../Docs/Product/WebUI/M1_4_Alias_Rollup_Week2_Controlled_2026_02_13.json'
  );
  await fs.writeFile(outputPath, `${JSON.stringify(capture, null, 2)}\n`, 'utf8');

  testInfo.annotations.push({
    type: 'alias-rollup-output',
    description: outputPath,
  });

  console.log(`\\nAlias rollup written to: ${outputPath}`);
  console.log(`Alias rollup total redirects: ${totalRedirects}`);
});
