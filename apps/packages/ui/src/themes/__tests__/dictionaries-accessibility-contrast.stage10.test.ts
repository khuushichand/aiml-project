...
const DICTIONARIES_CONTRAST_REQUIREMENTS: ContrastRequirement[] = [
...
describe("chat dictionaries stage 10 contrast coverage", () => {
  const presets = getBuiltinPresets()

  it("meets WCAG thresholds for dictionary text and focus token pairings", () => {
    const failures: string[] = []

    for (const preset of presets) {
      for (const [mode, tokens] of Object.entries(preset.palette)) {
        for (const requirement of DICTIONARIES_CONTRAST_REQUIREMENTS) {
  ...
  it("keeps documented minimum contrast floors stable for dictionaries", () => {
