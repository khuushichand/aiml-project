...
const mediaItems = Array.from({ length: 6 }).map((_, idx) => ({
  id: idx + 1,
  title: `Item ${idx + 1}`,
...
vi.mock("antd", async (importOriginal) => {
  ...
  const Modal = ({ open, title, onCancel, afterOpenChange, footer, children }: any) => {
    React.useEffect(() => {
...
describe("MediaReviewPage stage6 focus recovery", () => {
  ...
  const extractUndoHandler = (): (() => void) => {
    ...
    const node = latestCall[0] as React.ReactElement
    const children = React.Children.toArray(node.props.children)
    const buttonEl = children.find(
      (child): child is React.ReactElement => React.isValidElement(child) && typeof child.props?.onClick === "function"
    )
    if (!buttonEl) throw new Error("Missing undo handler")
    return buttonEl.props.onClick as () => void
  }
  ...
  it("returns focus to previously focused result row after undo restore", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("0 / 30 selected")).toBeInTheDocument()
      expect(mocks.messageInfo).toHaveBeenCalled()
    })

    const searchInput = screen.getByRole("textbox", { name: /search media/i })
    searchInput.focus()
    expect(searchInput).toHaveFocus()

    const undo = extractUndoHandler()
    undo()

    await waitFor(() => {
      expect(screen.getByText("1 / 30 selected")).toBeInTheDocument()
      expect(getResultRowByTitle("Item 4")).toHaveFocus()
    })
  })
