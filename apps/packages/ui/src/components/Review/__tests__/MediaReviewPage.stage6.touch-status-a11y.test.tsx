...
const mediaItems = Array.from({ length: 30 }).map((_, idx) => ({
  id: idx + 1,
  title: `Item ${idx + 1}`,
...
describe("MediaReviewPage stage6 touch targets and status semantics", () => {
  ...
  it("ensures key icon controls meet minimum 44x44 touch target", async () => {
    ...
    const helpButton = screen.getByRole("button", {
      name: /multi-item review keyboard shortcuts/i
    })
    const helpButtonClass = helpButton.getAttribute("class") || ""

    expect(helpButtonClass).toMatch(/min-h-\[44px\]|h-11/)
    expect(helpButtonClass).toMatch(/min-w-\[44px\]|w-11/)
  })
})
