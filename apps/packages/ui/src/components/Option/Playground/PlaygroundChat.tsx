...
export const PlaygroundChat = ({
  ...
  const firstUserIndex = React.useMemo(
    () =>
      messages.findIndex((msg) => msg?.role === "user" || msg?.isBot === false),
    [messages],
  );
  ...
  // Virtualization
  const [chatVirtualization] = useStorage("chatVirtualization", true);
        ...
        <div className="ml-10 space-y-2 border-l border-dashed border-border pl-4">
          ...
          {clusterModelKeys.length > 1 && (
            <div className="mb-2 flex flex-wrap items-center gap-2 text-[10px] text-text-muted">
              ...
              <button
                type="button"
                onClick={() => setNormalizedPreview(!normalizedPreviewEnabled)}
                title={
          ...
          {visibleReplyItems.map(({ index, message, modelKey }) => {
            ...
            const providerKey = getModelProvider(modelKey);
            const providerLabel =
              tldwModels.getProviderDisplayName(providerKey);
            ...
            return (
              ...
              >
                ...
                <PlaygroundMessage
                  ...
                  compareErrorModelLabel={
                    hasError ? getModelLabel(modelKey) : undefined
                  }
  ...
  const isVirtualized = chatVirtualization && Boolean(scrollParentRef?.current);
        ...
        {isVirtualized ? (
          ...
          >
            {virtualizer.getVirtualItems().map((vr) => (
              ...
              >
                {renderBlock(blocks[vr.index], vr.index)}
              </div>
            ))}
          </div>
        ) : (
