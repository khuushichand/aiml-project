...
export const TakeQuizTab: React.FC<TakeQuizTabProps> = ({
  ...
  const [page, setPage] = React.useState(() => {
    ...
    try {
      ...
      return typeof parsed.page === "number" && parsed.page > 0
        ? parsed.page
        : 1;
    } catch {
      return 1;
  ...
  const [sortBy, setSortBy] = React.useState<QuizSortKey>(() => {
    ...
    try {
      ...
      if (
        ...
        parsed.sortBy === "date_desc" ||
        parsed.sortBy === "questions_desc"
      ) {
  ...
  const [studyPoolSizePreference, setStudyPoolSizePreference] =
    React.useState<StudyPoolSizePreference>(() => {
      ...
      try {
        ...
        if (
          typeof parsed.studyPoolSize === "number" &&
          Number.isFinite(parsed.studyPoolSize) &&
          parsed.studyPoolSize > 0
        ) {
  ...
  const [practiceQuestionTimerPreference, setPracticeQuestionTimerPreference] =
    React.useState<PracticeQuestionTimerPreference>(() => {
      if (typeof window === "undefined") return "off";
      try {
        const raw = window.sessionStorage.getItem(TAKE_QUIZ_LIST_PREFS_KEY);
        if (!raw) return "off";
        const parsed = JSON.parse(raw) as {
          practiceQuestionTimerSeconds?: number | "off";
        };
  ...
  const [activeSessionMode, setActiveSessionMode] =
    React.useState<TakeSessionMode | null>(null);
  ...
  const [autoSaveWarningDismissed, setAutoSaveWarningDismissed] =
    React.useState(false);
  const [assignmentAlertDismissed, setAssignmentAlertDismissed] =
    React.useState(false);
  const [focusedQuestionId, setFocusedQuestionId] = React.useState<
    number | null
  >(null);
  const [unansweredQuestionNumbers, setUnansweredQuestionNumbers] =
  ...
  const [hintUsedByQuestionId, setHintUsedByQuestionId] = React.useState<
    Record<number, boolean>
  >({});
  const [practiceFeedbackByQuestionId, setPracticeFeedbackByQuestionId] =
    React.useState<Record<number, boolean | null>>({});
  const [
    practiceQuestionTimerRemainingSeconds,
    setPracticeQuestionTimerRemainingSeconds,
  ] = React.useState<number | null>(null);
  const [studySessionShuffleSeed, setStudySessionShuffleSeed] = React.useState<
    number | null
  >(null);
  const [queuedSubmission, setQueuedSubmission] =
    React.useState<QueuedQuizSubmission | null>(null);
  const [isRetryingQueuedSubmission, setIsRetryingQueuedSubmission] =
    React.useState(false);
  const [
    submissionQueueStorageUnavailable,
    setSubmissionQueueStorageUnavailable,
  ] = React.useState(false);
  const [highlightedQuizId, setHighlightedQuizId] = React.useState<
    number | null
  >(null);
  ...
  const questionRefs = React.useRef<Map<number, HTMLDivElement | null>>(
    new Map(),
  );
  ...
  const { data: quizDetails } = useQuizQuery(detailQuizId, {
    enabled: detailQuizId != null,
  });
  ...
  const { storageUnavailable, restoreSavedAnswers, clearSavedProgress } =
    useQuizAutoSave(
      attempt?.id ?? null,
      attempt?.quiz_id ?? activeQuizId,
      answers,
      setAnswers,
    );
  ...
  const timerAnnouncement = React.useMemo(() => {
    if (!timerState || timerState.isExpired || timerState.totalSeconds <= 0)
      return "";
  ...
  const statusAnnouncement = React.useMemo(() => {
    if (result) {
      ...
      return t("option:quiz.resultAnnouncement", {
        defaultValue:
          "Quiz submitted. Score {{score}} out of {{total}} ({{percent}} percent).",
        score,
        total,
        percent: percentage,
      });
    ...
    if (queuedSubmission) {
      return queuedSubmission.allowPartial
        ...
        : t("option:quiz.queueAnnouncement", {
            defaultValue:
              "Submission failed and is queued locally. Retry is available.",
          });
    }
    return "";
  }, [queuedSubmission, result, t]);

  const getSubmissionErrorMessage = React.useCallback(
    (error: unknown) => {
  ...
  const persistQueuedSubmission = React.useCallback(
    async (next: QueuedQuizSubmission) => {
  ...
  const clearCurrentQueuedSubmission = React.useCallback(
    async (attemptId: number | null | undefined) => {
      if (attemptId == null) return;
      await clearQueuedQuizSubmission(attemptId);
      setQueuedSubmission((current) =>
  ...
  const retryQueuedSubmission = React.useCallback(
    async (source: "manual" | "online") => {
      if (
        !queuedSubmission ||
        ...
        messageApi.success(
          source === "online"
            ? t("option:quiz.submitRetryAutoSuccess", {
                defaultValue:
                  "Connection restored. Queued quiz submission sent.",
              })
        ...
        messageApi.error(
          source === "online"
            ? t("option:quiz.submitRetryAutoFailed", {
                defaultValue:
                  "Auto-retry failed. Your answers are still queued locally.",
              })
            : t("option:quiz.submitRetryFailed", {
                defaultValue:
                  "Retry failed. Your answers are still queued locally.",
              }),
        );
      } finally {
  ...
  const handleStartStudySession = async (
    ...
    try {
      ...
      setStudySessionShuffleSeed(
        nextSessionSeed === 0 ? quizId : nextSessionSeed,
      );
  ...
  const getQuestionCorrectAnswer = (
    ...
    const raw = (
      question as QuestionPublic & { correct_answer?: AnswerValue | null }
    ).correct_answer;
  ...
  const getQuestionHintPenalty = (question: QuestionPublic): number => {
    const raw = (
      question as QuestionPublic & { hint_penalty_points?: number | null }
    ).hint_penalty_points;
  ...
  const getQuestionSourceCitations = (question: QuestionPublic) => {
    const raw = (question as QuestionPublic & { source_citations?: unknown })
      .source_citations;
  ...
  const isPracticeAnswerCorrect = (
    ...
    if (question.question_type === "multiple_choice") {
      ...
      if (!Number.isFinite(userIndex) || !Number.isFinite(correctIndex))
        return null;
  ...
  const hasMatchingAnswer = (
    ...
    return rows.every(
      ({ left }) =>
        typeof map[left] === "string" && map[left].trim().length > 0,
    );
  };
  ...
  const getOptionEntriesForQuestion = React.useCallback(
    (question: QuestionPublic): ShuffledOptionEntry[] => {
      if (
      ...
      if (optionShuffleSeed == null || options.length <= 1) {
        return options.map((label, originalIndex) => ({
          originalIndex,
          label,
        }));
      }
      return buildShuffledOptionEntries(
        options,
        question.id,
        optionShuffleSeed,
      );
  ...
  const formatQuestionAnswer = React.useCallback(
    (question: QuestionPublic, value: AnswerValue | undefined) => {
      if (value == null) {
  ...
  const setQuestionRef = React.useCallback(
    (questionId: number, node: HTMLDivElement | null) => {
  ...
  const handleSubmit = async (options?: { allowPartial?: boolean }) => {
    ...
    if (!options?.allowPartial && missing.length > 0) {
      ...
      messageApi.warning(
        t("option:quiz.answerAll", {
          defaultValue:
            "Please answer all questions before submitting. Unanswered: {{numbers}}",
    ...
    try {
      ...
      const previousQueue =
        queuedSubmission?.attemptId === attempt.id ? queuedSubmission : null;
          ...
          <Alert
            ...
            description={
              <div className="space-y-1">
                <QuizMarkdown
                  content={hint}
                  className="text-sm text-text-subtle [&>p]:my-1"
                />
                {hintPenaltyPoints > 0 && (
  ...
  const renderAnswerInput = (question: QuestionPublic) => {
            ...
            <Space orientation="vertical">
              {optionEntries.map((entry, displayIndex) => (
                <Radio key={entry.originalIndex} value={entry.originalIndex}>
                  {entry.label ||
                    `${t("option:quiz.option", { defaultValue: "Option" })} ${displayIndex + 1}`}
                </Radio>
              ))}
            </Space>
          </Radio.Group>
    ...
    if (question.question_type === "matching") {
      ...
      const rightOptions = Array.from(
        new Set(
          rows.map((row) => row.right).filter((value) => value.length > 0),
        ),
      );
            ...
            >
              ...
              {rightOptions.length > 0 ? (
                <Select
                  ...
                  placeholder={t("option:quiz.selectMatch", {
      ...
      return (
        <fieldset className="border-0 m-0 p-0">
          ...
          >
            <Space orientation="vertical">
              <Radio value="true">
                {t("option:quiz.true", { defaultValue: "True" })}
              </Radio>
              <Radio value="false">
                {t("option:quiz.false", { defaultValue: "False" })}
              </Radio>
            </Space>
          </Radio.Group>
        </fieldset>
      ...
      <div className="space-y-4">
          ...
          description={
            quizDetails?.passing_score != null
              ...
              : t("option:quiz.noPassingScoreDefault", {
                  defaultValue:
                    "No passing score set. Using default: {{score}}%.",
        ...
        <List
          ...
          renderItem={(question, index) => {
            ...
            const hintPenaltyPoints = Math.max(
              0,
              Number(answer?.hint_penalty_points ?? 0),
            );
            ...
            return (
              <List.Item>
                <div className="w-full space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      {isCorrect
                        ...
                        : t("option:quiz.incorrect", {
                            defaultValue: "Incorrect",
                          })}
                    </Tag>
                  </div>
                  <div className="text-sm text-text-muted">
                    {t("option:quiz.yourAnswer", {
                      defaultValue: "Your answer",
                    })}
                    :{" "}
                    <span className="font-medium">
                      {formatQuestionAnswer(question, userAnswer)}
                    </span>
                  </div>
                  <div className="text-sm text-text-muted">
                  ...
                  <SourceCitations
                    citations={
                      Array.isArray(answer?.source_citations)
        ...
        <Space>
          ...
          >
            <Button
              type="primary"
              onClick={() =>
                activeQuizId != null && requestGradedStart(activeQuizId)
              }
  ...
  const formatQuizTimeLimit = (timeLimitSeconds: number | null | undefined) => {
    ...
    if (minutes === 0) {
      return t("option:quiz.secondsShort", {
  ...
  const getQuizDifficultyLabel = (quiz: Quiz | null | undefined) => {
    const rawDifficulty = (
      quiz as (Quiz & { difficulty?: string | null }) | null
    )?.difficulty;
    if (!rawDifficulty) {
  ...
  const lastScoreByQuizId = React.useMemo(() => {
    ...
    sortedAttempts.forEach((entry) => {
      ...
      const percentage = Math.round(
        ((entry.score ?? 0) / entry.total_possible) * 100,
      );
  ...
  const highlightNotice = React.useMemo(() => {
    ...
    if (navigationSource === "generate") {
      return t("option:quiz.generatedHighlightNotice", {
        defaultValue:
          "Quiz generated successfully: {{name}}. It is highlighted below.",
    ...
    if (navigationSource === "create") {
      return t("option:quiz.createdHighlightNotice", {
        defaultValue:
          "Quiz created successfully: {{name}}. It is highlighted below.",
        name: highlightedQuiz.name,
      });
    }
  ...
  const renderAssignmentAlert = () => {
    ...
    const assignmentAlertTitle = t("option:quiz.assignmentActive", {
    ...
    >
      <div className="space-y-3">
        <Typography.Text className="block text-text-muted">
          {pendingQuiz?.name ??
            t("option:quiz.selectedQuiz", { defaultValue: "Selected quiz" })}
        </Typography.Text>
        <Descriptions size="small" bordered column={1}>
          ...
          <Descriptions.Item
            label={t("option:quiz.passingScore", {
  ...
  const renderSubmissionQueueAlert = () => {
    ...
    const queuedAtLabel = new Date(
      queuedSubmission.queuedAt,
    ).toLocaleTimeString();
    return (
      <Alert
        type="error"
          ...
          <div className="space-y-2">
            <Typography.Text className="block text-xs text-text-muted">
              {t("option:quiz.submitQueueMeta", {
                defaultValue: "Queued at {{time}}. Retry attempts: {{count}}.",
                time: queuedAtLabel,
        ...
        action={
          <Button
            ...
            loading={
              submitAttemptMutation.isPending || isRetryingQueuedSubmission
            }
            disabled={
              !isOnline ||
              submitAttemptMutation.isPending ||
          ...
          >
            {t("option:quiz.retrySubmission", {
              defaultValue: "Retry submission",
            })}
          </Button>
        }
      />
  ...
  if (
    (activeSessionMode === "practice" || activeSessionMode === "review") &&
    studySessionLoading
  ) {
    return (
              ...
              description={
                studyPoolSizePreference !== "all"
                  ? t("option:quiz.poolSessionSummary", {
                      defaultValue:
                        "Showing {{shown}} questions from a randomized pool.",
              ...
              {practiceQuestionTimerPreference !== "off" &&
                practiceTimerDisplay && (
                  <Tag
                    color={
                      practiceQuestionTimerRemainingSeconds != null &&
                      practiceQuestionTimerRemainingSeconds <= 10
                        ? "red"
                        : "gold"
                    }
            ...
            <Card size="small">
              <div className="space-y-2">
                <Typography.Text className="text-xs text-text-muted">
                  {t("option:quiz.questionNavigator", {
                    ...
                    <div
                      ...
                      className={`w-full space-y-2 rounded-md p-2 transition-colors ${
                        focusedQuestionId === question.id
                          ? "border border-warning/60 bg-warning/10"
                          : ""
                      }`}
                    >
                      <div className="font-medium">
                          ...
                          description={
                            !feedback ? (
                              <div className="space-y-1">
                                ...
                                <SourceCitations
                                  citations={getQuestionSourceCitations(
                                    question,
                                  )}
                                  fallbackMediaId={
                                    quizDetails?.media_id ?? null
                                  }
                                />
                              </div>
                            ) : undefined
        ...
        >
          <div className="flex gap-2 justify-center">
            <Button
              ...
              className={TOUCH_TARGET_CLASS}
            >
              {t("option:quiz.generateFromMedia", {
                defaultValue: "Generate from Media",
              })}
            </Button>
            <Button onClick={onNavigateToCreate} className={TOUCH_TARGET_CLASS}>
              {t("option:quiz.createManually", {
  ...
  return (
    <div className="space-y-4">
      ...
      {renderStartConfirmationModal()}
      <div className="text-sm text-text-muted">
        {t("option:quiz.selectQuiz", {
          defaultValue: "Select a quiz to begin",
        })}
      </div>
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <Input.Search
          ...
          placeholder={t("option:quiz.searchPlaceholder", {
            defaultValue: "Search quizzes...",
          })}
        ...
        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row">
          <Select
            ...
            options={[
              {
                value: "graded",
                label: t("option:quiz.modeGraded", {
                  defaultValue: "Mode: Graded",
                }),
              },
              {
                value: "practice",
                label: t("option:quiz.modePractice", {
                  defaultValue: "Mode: Practice",
                }),
              },
              {
                value: "review",
                label: t("option:quiz.modeReview", {
                  defaultValue: "Mode: Review",
                }),
              },
            ]}
          />
          {modePreference !== "graded" && (
            <Select
              ...
              options={[
                {
                  value: "all",
                  label: t("option:quiz.poolAll", {
                    defaultValue: "Question pool: All",
                  }),
                },
                {
                  value: 5,
                  label: t("option:quiz.poolFive", {
                    defaultValue: "Question pool: 5",
                  }),
                },
                {
                  value: 10,
                  label: t("option:quiz.poolTen", {
                    defaultValue: "Question pool: 10",
                  }),
                },
                {
                  value: 20,
                  label: t("option:quiz.poolTwenty", {
                    defaultValue: "Question pool: 20",
                  }),
                },
              ]}
            />
          ...
          {modePreference === "practice" && (
            <Select
              ...
              onChange={(nextValue) =>
                setPracticeQuestionTimerPreference(nextValue)
              }
              options={[
                {
                  value: "off",
                  label: t("option:quiz.practiceTimerOff", {
                    defaultValue: "Question timer: Off",
                  }),
                },
                {
                  value: 30,
                  label: t("option:quiz.practiceTimerThirty", {
                    defaultValue: "Question timer: 30s",
                  }),
                },
                {
                  value: 60,
                  label: t("option:quiz.practiceTimerSixty", {
                    defaultValue: "Question timer: 60s",
                  }),
                },
                {
                  value: 90,
                  label: t("option:quiz.practiceTimerNinety", {
                    defaultValue: "Question timer: 90s",
                  }),
                },
              ]}
            />
          ...
          <Select
            ...
            options={[
              {
                value: "date_desc",
                label: t("option:quiz.sortNewest", {
                  defaultValue: "Newest first",
                }),
              },
              {
                value: "name_asc",
                label: t("option:quiz.sortName", {
                  defaultValue: "Name (A-Z)",
                }),
              },
              {
                value: "questions_desc",
                label: t("option:quiz.sortQuestionCount", {
                  defaultValue: "Most questions",
                }),
              },
            ]}
          />
      ...
      <List
        ...
        pagination={{
          ...
          locale: {
            items_per_page: t("option:quiz.itemsPerPage", {
              ...
              <Card
                ...
                actions={[
                  <Button
                    ...
                    disabled={
                      startAttemptMutation.isPending || studySessionLoading
                    }
                    onClick={() => {
                      setHighlightedQuizId(null);
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {quiz.passing_score != null && (
                          <Tag color="blue">
                            {t("option:quiz.passingScoreLabel", {
                              defaultValue: "Pass",
                            })}
                            : {quiz.passing_score}%
                          </Tag>
                        )}
                        {(quiz as Quiz & { difficulty?: string | null })
                          .difficulty && (
                          <Tag color="purple">
                        ...
                        {sourceSummary.media > 0 && (
                          <Tag
                            color="green"
                            data-testid={`take-quiz-source-media-${quiz.id}`}
                          >
                        ...
                        {sourceSummary.notes > 0 && (
                          <Tag
                            color="cyan"
                            data-testid={`take-quiz-source-notes-${quiz.id}`}
                          >
                        ...
                        {sourceSummary.flashcards > 0 && (
                          <Tag
                            color="magenta"
                            data-testid={`take-quiz-source-flashcards-${quiz.id}`}
                          >
                        ...
                        {lastScoreByQuizId.has(quiz.id) && (
                          <Tag color="geekblue">
                            {t("option:quiz.lastScore", {
                              defaultValue: "Last score: {{score}}%",
                              score: lastScoreByQuizId.get(quiz.id),
                            })}
                          </Tag>
                        )}
                      </div>
                      {formatQuizDate(quiz.created_at) && (
                        <p className="text-xs text-text-subtle">
                          {t("option:quiz.createdOn", {
                            defaultValue: "Created: {{date}}",
                            date: formatQuizDate(quiz.created_at),
                          })}
                        </p>
                      )}
                    </div>
