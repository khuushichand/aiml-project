...
- Introduce a discovery step that reads `/api/v1/config/docs-info` for safe hints (mode, defaults, endpoints, providers) to seed onboarding and capability gating.
...
- Add: `probeWebUIConfig(baseUrl)` → `{ mode, authHints, chatDefaults, endpoints, providers }`
  - Reads `/api/v1/config/docs-info` (absolute URL). Caches in storage.
  - Use to:
...
- `/api/v1/config/docs-info` is a safe hint source; do not treat it as authoritative for secrets.
...
## References
- WebUI config hints at mode/auth/endpoints/defaults: `/api/v1/config/docs-info`
