...
This extension is part of a bun workspace monorepo that shares UI code with the Next.js web UI.

**Key directories:**
- `packages/ui/src/` — Shared components, hooks, services, routes, stores
- `extension/` — WXT build config, manifests, thin entry wrappers
- `tldw-frontend/` — Next.js web app (consumes shared UI)

**For feature parity guidelines, see:** [../DEVELOPMENT.md](../DEVELOPMENT.md)

> **Path note:** Most `src/...` paths in this document refer to `packages/ui/src/...` in the monorepo layout.
...
The shared UI code lives in `packages/ui/src/`. The `extension/` directory contains WXT-specific config and thin entry wrappers.

```
apps/
├── extension/                 # This directory - WXT extension build
...
├── packages/ui/src/           # Shared UI source (core of both platforms)
│   ├── entries/               # Browser entry points (background, sidepanel, options)
...
└── tldw-frontend/             # Next.js web app (separate build)
```
...
import { browser } from "wxt/browser"  // Browser APIs
```
