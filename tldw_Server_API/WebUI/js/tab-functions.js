...
async function chatLoadConversations(query) {
    try {
        ...
        items.forEach((it) => {
            ...
            const btnOpen = document.createElement('button'); btnOpen.className='btn btn-sm'; btnOpen.textContent='Open'; btnOpen.addEventListener('click', ()=>{ chatOpenConversation(id); });
            const btnRename = document.createElement('button'); btnRename.className='btn btn-sm'; btnRename.textContent='Rename'; btnRename.addEventListener('click', async ()=>{ const nt=prompt('New title', title); if(!nt) return; try{ await apiClient.makeRequest('PUT', `/api/v1/chats/${id}`, { body: { title: nt } }); chatLoadConversations(q); }catch(e){ if(Toast?.error) Toast.error('Rename failed'); }});
            const btnStar = document.createElement('button'); btnStar.className='btn btn-sm'; btnStar.textContent= (it.rating && it.rating >=5) ? '★' : '☆'; btnStar.addEventListener('click', async ()=>{ try{ const newRating = (it.rating && it.rating >=5) ? 0 : 5; await apiClient.makeRequest('PUT', `/api/v1/chats/${id}`, { body:{ rating:newRating } }); chatLoadConversations(q);}catch(e){ if(Toast?.error) Toast.error('Update failed'); }});
            const btnDel = document.createElement('button'); btnDel.className='btn btn-sm btn-danger'; btnDel.textContent='Delete'; btnDel.addEventListener('click', async ()=>{ if(!confirm('Delete this conversation?')) return; try{ await apiClient.makeRequest('DELETE', `/api/v1/chats/${id}`); chatLoadConversations(q); if (String(chatConversationId)===id) chatSetConversationId(null);}catch(e){ if(Toast?.error) Toast.error('Delete failed'); }});
            ...
            row.addEventListener('click', ()=>{ chatOpenConversation(id); });
