...
- Purpose: OpenAI-compatible embeddings API with production safeguards and a Jobs-backed worker for media embeddings. Focus on reliability (circuit breaker, retries), performance (TTL cache, connection pooling, batching), and observability (metrics, health, DLQ tools).
- Capabilities:
...
- Architecture & Data Flow:
  ...
  - Jobs worker: `services/jobs_worker.py` (media embeddings via core Jobs)
- Key Components:
  ...
  - Services — `services/jobs_worker.py`, `services/vector_compactor.py`
- Data Models & DB:
