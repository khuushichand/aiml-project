...
class WorkflowEngine:
    ...
    async def _run_step_adapter(
        ...
        if step_type == "notes":
            from tldw_Server_API.app.core.Workflows.adapters import run_notes_adapter
            return await run_notes_adapter(step_cfg, ctx)
        if step_type == "prompts":
            from tldw_Server_API.app.core.Workflows.adapters import run_prompts_adapter
            return await run_prompts_adapter(step_cfg, ctx)
        if step_type == "chunking":
            from tldw_Server_API.app.core.Workflows.adapters import run_chunking_adapter
            return await run_chunking_adapter(step_cfg, ctx)
        if step_type == "web_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_web_search_adapter
            return await run_web_search_adapter(step_cfg, ctx)
        if step_type == "collections":
            from tldw_Server_API.app.core.Workflows.adapters import run_collections_adapter
            return await run_collections_adapter(step_cfg, ctx)
        if step_type == "chatbooks":
            from tldw_Server_API.app.core.Workflows.adapters import run_chatbooks_adapter
            return await run_chatbooks_adapter(step_cfg, ctx)
        if step_type == "evaluations":
            from tldw_Server_API.app.core.Workflows.adapters import run_evaluations_adapter
            return await run_evaluations_adapter(step_cfg, ctx)
        if step_type == "claims_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_claims_extract_adapter
            return await run_claims_extract_adapter(step_cfg, ctx)
        if step_type == "character_chat":
            from tldw_Server_API.app.core.Workflows.adapters import run_character_chat_adapter
            return await run_character_chat_adapter(step_cfg, ctx)
        if step_type == "moderation":
            from tldw_Server_API.app.core.Workflows.adapters import run_moderation_adapter
            return await run_moderation_adapter(step_cfg, ctx)
        if step_type == "sandbox_exec":
            from tldw_Server_API.app.core.Workflows.adapters import run_sandbox_exec_adapter
            return await run_sandbox_exec_adapter(step_cfg, ctx)
        if step_type == "image_gen":
            from tldw_Server_API.app.core.Workflows.adapters import run_image_gen_adapter
            return await run_image_gen_adapter(step_cfg, ctx)
        if step_type == "summarize":
            from tldw_Server_API.app.core.Workflows.adapters import run_summarize_adapter
            return await run_summarize_adapter(step_cfg, ctx)
        if step_type == "query_expand":
            from tldw_Server_API.app.core.Workflows.adapters import run_query_expand_adapter
            return await run_query_expand_adapter(step_cfg, ctx)
        if step_type == "rerank":
            from tldw_Server_API.app.core.Workflows.adapters import run_rerank_adapter
            return await run_rerank_adapter(step_cfg, ctx)
        if step_type == "citations":
            from tldw_Server_API.app.core.Workflows.adapters import run_citations_adapter
            return await run_citations_adapter(step_cfg, ctx)
        if step_type == "ocr":
            from tldw_Server_API.app.core.Workflows.adapters import run_ocr_adapter
            return await run_ocr_adapter(step_cfg, ctx)
        if step_type == "pdf_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_pdf_extract_adapter
            return await run_pdf_extract_adapter(step_cfg, ctx)
        if step_type == "voice_intent":
            from tldw_Server_API.app.core.Workflows.adapters import run_voice_intent_adapter
            return await run_voice_intent_adapter(step_cfg, ctx)
        # Tier 1: Research Automation
        if step_type == "query_rewrite":
            from tldw_Server_API.app.core.Workflows.adapters import run_query_rewrite_adapter
            return await run_query_rewrite_adapter(step_cfg, ctx)
        if step_type == "hyde_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_hyde_generate_adapter
            return await run_hyde_generate_adapter(step_cfg, ctx)
        if step_type == "semantic_cache_check":
            from tldw_Server_API.app.core.Workflows.adapters import run_semantic_cache_check_adapter
            return await run_semantic_cache_check_adapter(step_cfg, ctx)
        if step_type == "search_aggregate":
            from tldw_Server_API.app.core.Workflows.adapters import run_search_aggregate_adapter
            return await run_search_aggregate_adapter(step_cfg, ctx)
        if step_type == "entity_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_entity_extract_adapter
            return await run_entity_extract_adapter(step_cfg, ctx)
        if step_type == "bibliography_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_bibliography_generate_adapter
            return await run_bibliography_generate_adapter(step_cfg, ctx)
        if step_type == "document_table_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_document_table_extract_adapter
            return await run_document_table_extract_adapter(step_cfg, ctx)
        if step_type == "audio_diarize":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_diarize_adapter
            return await run_audio_diarize_adapter(step_cfg, ctx)
        # Tier 2: Learning/Education
        if step_type == "flashcard_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_flashcard_generate_adapter
            return await run_flashcard_generate_adapter(step_cfg, ctx)
        if step_type == "quiz_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_quiz_generate_adapter
            return await run_quiz_generate_adapter(step_cfg, ctx)
        if step_type == "quiz_evaluate":
            from tldw_Server_API.app.core.Workflows.adapters import run_quiz_evaluate_adapter
            return await run_quiz_evaluate_adapter(step_cfg, ctx)
        if step_type == "outline_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_outline_generate_adapter
            return await run_outline_generate_adapter(step_cfg, ctx)
        if step_type == "glossary_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_glossary_extract_adapter
            return await run_glossary_extract_adapter(step_cfg, ctx)
        if step_type == "mindmap_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_mindmap_generate_adapter
            return await run_mindmap_generate_adapter(step_cfg, ctx)
        if step_type == "eval_readability":
            from tldw_Server_API.app.core.Workflows.adapters import run_eval_readability_adapter
            return await run_eval_readability_adapter(step_cfg, ctx)
        # Tier 3: Data Processing
        if step_type == "json_transform":
            from tldw_Server_API.app.core.Workflows.adapters import run_json_transform_adapter
            return await run_json_transform_adapter(step_cfg, ctx)
        if step_type == "json_validate":
            from tldw_Server_API.app.core.Workflows.adapters import run_json_validate_adapter
            return await run_json_validate_adapter(step_cfg, ctx)
        if step_type == "csv_to_json":
            from tldw_Server_API.app.core.Workflows.adapters import run_csv_to_json_adapter
            return await run_csv_to_json_adapter(step_cfg, ctx)
        if step_type == "json_to_csv":
            from tldw_Server_API.app.core.Workflows.adapters import run_json_to_csv_adapter
            return await run_json_to_csv_adapter(step_cfg, ctx)
        if step_type == "regex_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_regex_extract_adapter
            return await run_regex_extract_adapter(step_cfg, ctx)
        if step_type == "text_clean":
            from tldw_Server_API.app.core.Workflows.adapters import run_text_clean_adapter
            return await run_text_clean_adapter(step_cfg, ctx)
        if step_type == "xml_transform":
            from tldw_Server_API.app.core.Workflows.adapters import run_xml_transform_adapter
            return await run_xml_transform_adapter(step_cfg, ctx)
        if step_type == "template_render":
            from tldw_Server_API.app.core.Workflows.adapters import run_template_render_adapter
            return await run_template_render_adapter(step_cfg, ctx)
        if step_type == "batch":
            from tldw_Server_API.app.core.Workflows.adapters import run_batch_adapter
            return await run_batch_adapter(step_cfg, ctx)
        # Tier 4: Workflow Orchestration
        if step_type == "workflow_call":
            from tldw_Server_API.app.core.Workflows.adapters import run_workflow_call_adapter
            return await run_workflow_call_adapter(step_cfg, ctx)
        if step_type == "parallel":
            from tldw_Server_API.app.core.Workflows.adapters import run_parallel_adapter
            return await run_parallel_adapter(step_cfg, ctx)
        if step_type == "cache_result":
            from tldw_Server_API.app.core.Workflows.adapters import run_cache_result_adapter
            return await run_cache_result_adapter(step_cfg, ctx)
        if step_type == "retry":
            from tldw_Server_API.app.core.Workflows.adapters import run_retry_adapter
            return await run_retry_adapter(step_cfg, ctx)
        if step_type == "checkpoint":
            from tldw_Server_API.app.core.Workflows.adapters import run_checkpoint_adapter
            return await run_checkpoint_adapter(step_cfg, ctx)
        # Tier 5: External Integrations
        if step_type == "s3_upload":
            from tldw_Server_API.app.core.Workflows.adapters import run_s3_upload_adapter
            return await run_s3_upload_adapter(step_cfg, ctx)
        if step_type == "s3_download":
            from tldw_Server_API.app.core.Workflows.adapters import run_s3_download_adapter
            return await run_s3_download_adapter(step_cfg, ctx)
        if step_type == "github_create_issue":
            from tldw_Server_API.app.core.Workflows.adapters import run_github_create_issue_adapter
            return await run_github_create_issue_adapter(step_cfg, ctx)
        # Tier 6: Agentic Support
        if step_type == "llm_with_tools":
            from tldw_Server_API.app.core.Workflows.adapters import run_llm_with_tools_adapter
            return await run_llm_with_tools_adapter(step_cfg, ctx)
        if step_type == "llm_critique":
            from tldw_Server_API.app.core.Workflows.adapters import run_llm_critique_adapter
            return await run_llm_critique_adapter(step_cfg, ctx)
        if step_type == "context_build":
            from tldw_Server_API.app.core.Workflows.adapters import run_context_build_adapter
            return await run_context_build_adapter(step_cfg, ctx)
        # Phase 2: Group A - Individual Utility Nodes
        if step_type == "document_merge":
            from tldw_Server_API.app.core.Workflows.adapters import run_document_merge_adapter
            return await run_document_merge_adapter(step_cfg, ctx)
        if step_type == "document_diff":
            from tldw_Server_API.app.core.Workflows.adapters import run_document_diff_adapter
            return await run_document_diff_adapter(step_cfg, ctx)
        if step_type == "markdown_to_html":
            from tldw_Server_API.app.core.Workflows.adapters import run_markdown_to_html_adapter
            return await run_markdown_to_html_adapter(step_cfg, ctx)
        if step_type == "html_to_markdown":
            from tldw_Server_API.app.core.Workflows.adapters import run_html_to_markdown_adapter
            return await run_html_to_markdown_adapter(step_cfg, ctx)
        if step_type == "keyword_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_keyword_extract_adapter
            return await run_keyword_extract_adapter(step_cfg, ctx)
        if step_type == "sentiment_analyze":
            from tldw_Server_API.app.core.Workflows.adapters import run_sentiment_analyze_adapter
            return await run_sentiment_analyze_adapter(step_cfg, ctx)
        if step_type == "language_detect":
            from tldw_Server_API.app.core.Workflows.adapters import run_language_detect_adapter
            return await run_language_detect_adapter(step_cfg, ctx)
        if step_type == "topic_model":
            from tldw_Server_API.app.core.Workflows.adapters import run_topic_model_adapter
            return await run_topic_model_adapter(step_cfg, ctx)
        if step_type == "token_count":
            from tldw_Server_API.app.core.Workflows.adapters import run_token_count_adapter
            return await run_token_count_adapter(step_cfg, ctx)
        if step_type == "context_window_check":
            from tldw_Server_API.app.core.Workflows.adapters import run_context_window_check_adapter
            return await run_context_window_check_adapter(step_cfg, ctx)
        if step_type == "llm_compare":
            from tldw_Server_API.app.core.Workflows.adapters import run_llm_compare_adapter
            return await run_llm_compare_adapter(step_cfg, ctx)
        if step_type == "image_describe":
            from tldw_Server_API.app.core.Workflows.adapters import run_image_describe_adapter
            return await run_image_describe_adapter(step_cfg, ctx)
        if step_type == "report_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_report_generate_adapter
            return await run_report_generate_adapter(step_cfg, ctx)
        if step_type == "newsletter_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_newsletter_generate_adapter
            return await run_newsletter_generate_adapter(step_cfg, ctx)
        if step_type == "slides_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_slides_generate_adapter
            return await run_slides_generate_adapter(step_cfg, ctx)
        if step_type == "diagram_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_diagram_generate_adapter
            return await run_diagram_generate_adapter(step_cfg, ctx)
        if step_type == "email_send":
            from tldw_Server_API.app.core.Workflows.adapters import run_email_send_adapter
            return await run_email_send_adapter(step_cfg, ctx)
        if step_type == "screenshot_capture":
            from tldw_Server_API.app.core.Workflows.adapters import run_screenshot_capture_adapter
            return await run_screenshot_capture_adapter(step_cfg, ctx)
        if step_type == "schedule_workflow":
            from tldw_Server_API.app.core.Workflows.adapters import run_schedule_workflow_adapter
            return await run_schedule_workflow_adapter(step_cfg, ctx)
        if step_type == "timing_start":
            from tldw_Server_API.app.core.Workflows.adapters import run_timing_start_adapter
            return await run_timing_start_adapter(step_cfg, ctx)
        if step_type == "timing_stop":
            from tldw_Server_API.app.core.Workflows.adapters import run_timing_stop_adapter
            return await run_timing_stop_adapter(step_cfg, ctx)
        # Phase 2: Group B - Audio & Video Processing
        if step_type == "audio_normalize":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_normalize_adapter
            return await run_audio_normalize_adapter(step_cfg, ctx)
        if step_type == "audio_concat":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_concat_adapter
            return await run_audio_concat_adapter(step_cfg, ctx)
        if step_type == "audio_trim":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_trim_adapter
            return await run_audio_trim_adapter(step_cfg, ctx)
        if step_type == "audio_convert":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_convert_adapter
            return await run_audio_convert_adapter(step_cfg, ctx)
        if step_type == "audio_extract":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_extract_adapter
            return await run_audio_extract_adapter(step_cfg, ctx)
        if step_type == "audio_mix":
            from tldw_Server_API.app.core.Workflows.adapters import run_audio_mix_adapter
            return await run_audio_mix_adapter(step_cfg, ctx)
        if step_type == "video_thumbnail":
            from tldw_Server_API.app.core.Workflows.adapters import run_video_thumbnail_adapter
            return await run_video_thumbnail_adapter(step_cfg, ctx)
        if step_type == "video_trim":
            from tldw_Server_API.app.core.Workflows.adapters import run_video_trim_adapter
            return await run_video_trim_adapter(step_cfg, ctx)
        if step_type == "video_concat":
            from tldw_Server_API.app.core.Workflows.adapters import run_video_concat_adapter
            return await run_video_concat_adapter(step_cfg, ctx)
        if step_type == "video_convert":
            from tldw_Server_API.app.core.Workflows.adapters import run_video_convert_adapter
            return await run_video_convert_adapter(step_cfg, ctx)
        if step_type == "video_extract_frames":
            from tldw_Server_API.app.core.Workflows.adapters import run_video_extract_frames_adapter
            return await run_video_extract_frames_adapter(step_cfg, ctx)
        if step_type == "subtitle_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_subtitle_generate_adapter
            return await run_subtitle_generate_adapter(step_cfg, ctx)
        if step_type == "subtitle_translate":
            from tldw_Server_API.app.core.Workflows.adapters import run_subtitle_translate_adapter
            return await run_subtitle_translate_adapter(step_cfg, ctx)
        if step_type == "subtitle_burn":
            from tldw_Server_API.app.core.Workflows.adapters import run_subtitle_burn_adapter
            return await run_subtitle_burn_adapter(step_cfg, ctx)
        # Phase 2: Group C - Research & Academic
        if step_type == "arxiv_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_arxiv_search_adapter
            return await run_arxiv_search_adapter(step_cfg, ctx)
        if step_type == "arxiv_download":
            from tldw_Server_API.app.core.Workflows.adapters import run_arxiv_download_adapter
            return await run_arxiv_download_adapter(step_cfg, ctx)
        if step_type == "pubmed_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_pubmed_search_adapter
            return await run_pubmed_search_adapter(step_cfg, ctx)
        if step_type == "semantic_scholar_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_semantic_scholar_search_adapter
            return await run_semantic_scholar_search_adapter(step_cfg, ctx)
        if step_type == "google_scholar_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_google_scholar_search_adapter
            return await run_google_scholar_search_adapter(step_cfg, ctx)
        if step_type == "patent_search":
            from tldw_Server_API.app.core.Workflows.adapters import run_patent_search_adapter
            return await run_patent_search_adapter(step_cfg, ctx)
        if step_type == "doi_resolve":
            from tldw_Server_API.app.core.Workflows.adapters import run_doi_resolve_adapter
            return await run_doi_resolve_adapter(step_cfg, ctx)
        if step_type == "reference_parse":
            from tldw_Server_API.app.core.Workflows.adapters import run_reference_parse_adapter
            return await run_reference_parse_adapter(step_cfg, ctx)
        if step_type == "bibtex_generate":
            from tldw_Server_API.app.core.Workflows.adapters import run_bibtex_generate_adapter
            return await run_bibtex_generate_adapter(step_cfg, ctx)
        if step_type == "literature_review":
            from tldw_Server_API.app.core.Workflows.adapters import run_literature_review_adapter
            return await run_literature_review_adapter(step_cfg, ctx)
