...
class VideoConcatConfig(BaseAdapterConfig):
    ...
    output_format: str = Field("mp4", description="Output video format")
    transition: str | None = Field(None, description="Transition effect between clips")
...
class VideoConvertConfig(BaseAdapterConfig):
    ...
    video_codec: str | None = Field(None, description="Video codec (e.g., h264, vp9)")
    audio_codec: str | None = Field(None, description="Audio codec (e.g., aac, opus)")
    resolution: str | None = Field(None, description="Target resolution (e.g., '1920x1080')")
    bitrate: str | None = Field(None, description="Target video bitrate (e.g., '5M')")
    fps: int | None = Field(None, ge=1, le=120, description="Target frame rate")
...
class VideoThumbnailConfig(BaseAdapterConfig):
    ...
    file_uri: str = Field(..., description="file:// path to input video (required)")
    timestamp_ms: int | None = Field(None, ge=0, description="Timestamp to capture in milliseconds")
    timestamp_percent: float | None = Field(
        None, ge=0, le=100, description="Timestamp as percentage of video duration"
    )
    width: int | None = Field(None, ge=1, description="Output width in pixels")
    height: int | None = Field(None, ge=1, description="Output height in pixels")
...
class VideoExtractFramesConfig(BaseAdapterConfig):
    ...
    file_uri: str = Field(..., description="file:// path to input video (required)")
    fps: float | None = Field(None, ge=0.01, description="Frames per second to extract")
    count: int | None = Field(None, ge=1, description="Total number of frames to extract")
...
class SubtitleGenerateConfig(BaseAdapterConfig):
    ...
    max_line_length: int | None = Field(None, ge=20, description="Maximum characters per line")
...
class SubtitleTranslateConfig(BaseAdapterConfig):
    ...
    source_language: str | None = Field(None, description="Source language code (auto-detect if not specified)")
    provider: str | None = Field(None, description="Translation provider (openai, anthropic, etc.)")
    model: str | None = Field(None, description="Model to use for translation")
...
class SubtitleBurnConfig(BaseAdapterConfig):
    ...
    output_format: str = Field("mp4", description="Output video format")
    font_size: int | None = Field(None, ge=8, le=72, description="Font size for subtitles")
    font_name: str | None = Field(None, description="Font name for subtitles")
    position: Literal["bottom", "top", "center"] = Field("bottom", description="Subtitle position")
    margin_v: int | None = Field(None, ge=0, description="Vertical margin in pixels")
