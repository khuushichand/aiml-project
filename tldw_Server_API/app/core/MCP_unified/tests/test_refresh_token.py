...
async def test_refresh_token_rotation_flow():
    ...
    # Rotate
    resp = await refresh_endpoint(auth_request=AuthRefreshRequest(refresh_token=refresh, token_id=token_id))  # type: ignore[arg-type]
    ...
    with pytest.raises(HTTPException):
        await refresh_endpoint(auth_request=AuthRefreshRequest(refresh_token=refresh, token_id=token_id))  # type: ignore[arg-type]
