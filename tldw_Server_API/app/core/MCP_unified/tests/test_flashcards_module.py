...
class FakeFlashcardsDB:
    ...
    def list_flashcards(self, deck_id=None, tag=None, due_status="all", q=None, include_deleted=False, limit=100, offset=0, order_by="due_at"):
        items = list(self.cards.values())
        if deck_id is not None:
            items = [c for c in items if c.get("deck_id") == deck_id]
        return items[offset: offset + limit]

    def count_flashcards(self, deck_id=None, tag=None, due_status="all", q=None, include_deleted=False):
        return len(self.list_flashcards(deck_id=deck_id, tag=tag, due_status=due_status, q=q, include_deleted=include_deleted, limit=100000, offset=0))
    ...
    def export_flashcards_csv(self, deck_id=None, tag=None, q=None, delimiter=",", include_header=True, extended_header=False):
        return "front,back\nQ,A\n".encode("utf-8")
