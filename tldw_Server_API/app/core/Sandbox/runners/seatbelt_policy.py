...
_RESERVED_ENV_KEYS = {"HOME", "TMPDIR", "TMP", "TEMP", "PWD"}
...
def render_seatbelt_profile(
    *,
    ...
    lines = [
        ...
        '(import "system.sb")',
        "(allow process-fork)",
        '(allow process-exec (literal "{}"))'.format(command_path),
        "(allow signal (target self))",
        "(allow sysctl-read)",
        "(allow file-read-data file-read-metadata",
        '       (literal "{}")'.format(command_path),
        '       (subpath "{}")'.format(workspace_path),
        '       (subpath "{}")'.format(home_path),
        '       (subpath "{}")'.format(temp_path),
        '       (subpath "/bin")',
        '       (subpath "/usr/bin")',
        '       (subpath "/usr/lib")',
        '       (subpath "/System"))',
