...
class MediaDatabase:
    ...
    def soft_delete_data_table(self, table_id: int) -> bool:
        now = self._get_current_utc_timestamp_str()
        with self.transaction() as conn:
            cur = self._execute_with_connection(
                conn,
                ...
                """,
                (now, now, str(self.client_id), int(table_id)),
            )
            updated = int(getattr(cur, "rowcount", 0) or 0)
            if updated:
                self._soft_delete_data_table_children(conn, int(table_id), now)
        return bool(updated)

    def _soft_delete_data_table_children(self, conn, table_id: int, now: str) -> None:
    ...
    def insert_data_table_columns(self, table_id: int, columns: List[Dict[str, Any]]) -> int:
        if not columns:
            return 0
        now = self._get_current_utc_timestamp_str()
        rows: List[tuple] = []
    ...
    def soft_delete_data_table_columns(self, table_id: int) -> int:
        now = self._get_current_utc_timestamp_str()
        cur = self.execute_query(
            """
    ...
    def insert_data_table_rows(self, table_id: int, rows: List[Dict[str, Any]]) -> int:
        if not rows:
            return 0
        now = self._get_current_utc_timestamp_str()
    ...
    def soft_delete_data_table_rows(self, table_id: int) -> int:
        now = self._get_current_utc_timestamp_str()
        cur = self.execute_query(
            """
    ...
    def insert_data_table_sources(self, table_id: int, sources: List[Dict[str, Any]]) -> int:
        if not sources:
            return 0
        now = self._get_current_utc_timestamp_str()
        rows: List[tuple] = []
    ...
    def soft_delete_data_table_sources(self, table_id: int) -> int:
        now = self._get_current_utc_timestamp_str()
        cur = self.execute_query(
            """
    ...
    def add_media_with_keywords(
        ...
        Args:
            visibility: Content visibility ('personal', 'team', 'org'). Defaults to 'personal'. When overwriting an
                existing record, leaving this unset preserves the current visibility.
            owner_user_id: User ID of the content owner. Defaults to client_id if numeric. When overwriting an existing
                record, leaving this unset preserves the current owner.
        """
        ...
        # Validate visibility (normalize defaults later, based on insert vs overwrite semantics)
        valid_visibilities = ("personal", "team", "org")
        ...
        # Derive default owner_user_id from client_id (only used for inserts unless caller explicitly sets owner_user_id)
                ...
                # --- Path B: Record does not exist, perform INSERT ---
                    ...
                    with self._media_insert_lock:
                        ...
                        payload = _media_payload(
                            ...
                            owner_user_id=owner_user_id if owner_user_id is not None else derived_owner_user_id,
                        )
                        insert_sql = """
