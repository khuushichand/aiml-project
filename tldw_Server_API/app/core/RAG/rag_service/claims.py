"""
claims.py - Claim extraction and verification for Unified RAG

This module provides:
- ClaimExtractor interface with two simple implementations:
  - LLMBasedClaimExtractor: prompt-based extraction into decontextualized atomic propositions
  - HeuristicSentenceExtractor: fallback sentence splitter
- ClaimVerifier interface with a default HybridClaimVerifier:
  - Per-claim evidence retrieval using existing retriever
  - Entailment via LLM judge (JSON output) with simple numeric/date consistency checks

All I/O is async and designed to integrate into unified_pipeline.
"""
...
class LLMBasedClaimExtractor:
    ...
    def __init__(self, analyze_fn: Any):
        self._analyze = analyze_fn  # callable(api_name, input_data, custom_prompt_arg, api_key, system_message, temp)
...
class HybridClaimVerifier:
    """Verify with quick numeric/date checks, retrieve evidence, then LLM-judge entailment."""
    ...
    def __init__(self, analyze_fn: Any, nli_model: Optional[str] = None):
        ...
        try:
            ...
            # Lazy load in a thread to avoid blocking event loop
            def _load():
                try:
                    return pipeline("text-classification", model=model_name, return_all_scores=True)
                except Exception as e:
                    logger.warning(f"NLI model load failed ({model_name}): {e}. Consider providing a local path or pre-downloaded weights.")
                    return None
    ...
    def _nli_best_label(nli_scores: List[Dict[str, Any]]) -> Tuple[str, float]:
        # nli_scores is list of dicts: [{'label': 'ENTAILMENT','score':0.8}, ...]
    ...
    async def verify(
        ...
        # initial heuristic check against base docs
        nums_dates = _extract_numbers_and_dates(claim_text)

        candidate_docs: List[Document] = []
        try:
            if retrieve_fn is not None:
                # Prefer targeted per-claim retrieval
        ...
        # quick numeric/date filter to rank snippets higher that contain same numerics/dates
        ...
        nli_decision: Optional[Tuple[str, float, int]] = None  # (label, score, idx)
        if nli is not None:
            try:
                ...
                if best_tuple[1] >= conf_threshold:
                    ...
                    return ClaimVerification(
                        ...
                        citations=[],
                        rationale=f"NLI-{best_tuple[0]} with confidence {best_tuple[1]:.2f}",
                    )
                else:
        ...
        # LLM judge for entailment (hybrid or fallback)
        ...
        judge_prompt = (
            ...
            "Consider numeric/date consistency strictly. Return JSON as \n"
            "{\"label\": one of [\"supported\",\"refuted\",\"nei\"], \"confidence\": float in [0,1], \"rationale\": str}.\n\n"
            f"CLAIM: {claim_text}\n\nEVIDENCE:\n{evidence_text}"
        )
        label = "nei"
        confidence = 0.5
...
class ClaimsEngine:
    ...
    async def run(
        ...
        if claim_extractor == "claimify":
            ...
            # APS not yet integrated; fall back to LLM
            extractor = self.extractor_llm
        else:
            extractor = self.extractor_llm
        ...
        async def _verify_one(c: Claim) -> ClaimVerification:
            # Recreate verifier if model override provided
            if nli_model:
        ...
        # normalize to API friendly dicts
