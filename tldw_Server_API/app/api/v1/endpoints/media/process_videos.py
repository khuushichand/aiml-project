...
async def process_videos_endpoint(
    ...
    This keeps the HTTP contract, auth dependencies, and internal behavior
    identical while allowing the `/process-videos` route to live under the
    modular `media` package.
    """

    return await legacy_media.process_videos_endpoint(  # type: ignore[func-returns-value]
