...
def test_persona_sessions_list_and_detail_roundtrip(monkeypatch):
    manager = SessionManager()
    monkeypatch.setattr(persona_ep, "get_session_manager", lambda: manager)

    with _client_for_user(1) as client:
...
def test_persona_session_resume_rejects_ownership_mismatch(monkeypatch):
    manager = SessionManager()
    _ = manager.create(user_id="1", persona_id="research_assistant", resume_session_id="sess_owned_by_1")
    monkeypatch.setattr(persona_ep, "get_session_manager", lambda: manager)

    with _client_for_user(2) as client:
        resp = client.post(
...
def test_persona_session_detail_is_user_scoped(monkeypatch):
    manager = SessionManager()
    _ = manager.create(user_id="1", persona_id="research_assistant", resume_session_id="sess_scoped")
    monkeypatch.setattr(persona_ep, "get_session_manager", lambda: manager)
...
def test_persona_sessions_return_404_when_disabled(monkeypatch):
    manager = SessionManager()
    monkeypatch.setattr(persona_ep, "get_session_manager", lambda: manager)
