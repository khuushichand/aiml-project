...
def test_process_pdfs_upload_and_validate(page, server_url):
    ...
    pdf_path = Path(__file__).resolve().parents[2] / "Media_Ingestion_Modification" / "test_media" / "sample.pdf"
    ...
    # Basic assertions: processed structure and filename
    assert "processed_count" in resp_text or "results" in resp_text
    assert "sample.pdf" in resp_text
