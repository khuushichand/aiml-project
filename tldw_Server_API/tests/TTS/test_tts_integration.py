...
def can_run_local_model(model_name: str) -> bool:
    ...
    model_requirements = {
        ...
        "higgs": {"cpu": False, "mps": True, "cuda": True},  # Too slow on CPU  # Works on Apple Silicon  # Best on CUDA
        "dia": {"cpu": False, "mps": False, "cuda": True},  # Requires GPU  # Not optimized for MPS  # CUDA only
        "chatterbox": {
...
class TestAPITTSIntegration:
    ...
    async def test_openai_real_generation(self):
        ...
        request = TTSRequest(
            text="Hello, this is a test of OpenAI text to speech.", voice="nova", format=AudioFormat.MP3, speed=1.0
        )
    ...
    @pytest.mark.skipif(not os.getenv("ELEVENLABS_API_KEY"), reason="Requires ELEVENLABS_API_KEY environment variable")
    @pytest.mark.asyncio
    async def test_elevenlabs_streaming(self):
        ...
        request = TTSRequest(
            text="Testing ElevenLabs streaming capability.", voice="rachel", format=AudioFormat.MP3, stream=True
        )
    ...
    async def test_adapter_fallback_real(self):
        ...
        request = TTSRequest(text="Testing fallback mechanism.", voice="default")
...
class TestLocalTTSIntegration:
    ...
    async def test_kokoro_cpu_inference(self):
        ...
        request = TTSRequest(
            text="Testing Kokoro text to speech on local hardware.", voice="af_bella", format=AudioFormat.WAV
        )
    ...
    async def test_kokoro_gpu_inference(self):
        ...
        request = TTSRequest(
            text="Testing GPU accelerated Kokoro inference.", voice="af_sarah", format=AudioFormat.WAV, speed=1.2
        )
    ...
    async def test_higgs_voice_cloning(self):
        ...
        adapter = HiggsAdapter({"device": compute if compute != "mps" else "cpu"})
    ...
    @pytest.mark.skipif(not can_run_local_model("chatterbox"), reason="Requires appropriate hardware for Chatterbox")
    @pytest.mark.asyncio
    async def test_chatterbox_character_voices(self):
        """Test Chatterbox character voice generation"""
        adapter = ChatterboxAdapter({"model": "base", "device": get_compute_capability()})  # Use base model for testing
        ...
        for voice in voices:
            request = TTSRequest(text=f"Testing {voice} voice style.", voice=voice, format=AudioFormat.WAV)
...
class TestTTSPerformance:
    ...
    @pytest.mark.skipif(not can_run_local_model("kokoro"), reason="Requires Kokoro model")
    @pytest.mark.asyncio
    async def test_kokoro_performance_benchmark(self, benchmark):
        """Benchmark Kokoro generation speed"""
        adapter = KokoroAdapter({"model_type": "onnx", "device": get_compute_capability()})
        ...
        request = TTSRequest(
            text="The quick brown fox jumps over the lazy dog.", voice="af_bella", format=AudioFormat.WAV
        )
    ...
    async def test_api_latency_comparison(self):
        ...
        if os.getenv("OPENAI_API_KEY"):
            ...
            request = TTSRequest(text="Testing latency.", voice="nova", format=AudioFormat.MP3)
        ...
        if os.getenv("ELEVENLABS_API_KEY"):
            ...
            request = TTSRequest(text="Testing latency.", voice="rachel", format=AudioFormat.MP3)
...
class TestPlatformSpecific:
    ...
    async def test_macos_mps_acceleration(self):
        ...
        adapter = KokoroAdapter({"device": "mps"})

        success = await adapter.initialize()
        if not success:
            pytest.skip("Model initialization failed on MPS")

        request = TTSRequest(text="Testing Metal Performance Shaders acceleration.", voice="af_bella")
    ...
    async def test_windows_compatibility(self):
        ...
        if can_run_local_model("kokoro"):
            adapter = KokoroAdapter({"model_type": "onnx", "device": compute})

            success = await adapter.initialize()
            if success:
                request = TTSRequest(text="Testing on Windows platform.", voice="af_bella")
    ...
    async def test_linux_docker_compatibility(self):
        ...
        if is_container:
            ...
            adapter = KokoroAdapter({"model_type": "onnx", "device": "cpu"})
        else:
            # Use best available on host
            compute = get_compute_capability()
            adapter = KokoroAdapter({"device": compute})

        success = await adapter.initialize()
        if not success:
            pytest.skip("Model initialization failed in Linux environment")

        request = TTSRequest(text="Testing in Linux environment.", voice="af_bella")
