...
def test_is_safe_local_path_allows_relative_inside(tmp_path: Path) -> None:
    ...
    assert ingestion_persistence._is_safe_local_path(relative_path, base_dir)
...
def test_is_safe_local_path_allows_absolute_inside(tmp_path: Path) -> None:
    ...
    assert ingestion_persistence._is_safe_local_path(absolute_path, base_dir)
...
def test_is_safe_local_path_rejects_relative_escape(tmp_path: Path) -> None:
    ...
    assert not ingestion_persistence._is_safe_local_path(escaping_path, base_dir)
...
def test_is_safe_local_path_rejects_absolute_outside(tmp_path: Path) -> None:
    ...
    assert not ingestion_persistence._is_safe_local_path(outside_path, base_dir)
