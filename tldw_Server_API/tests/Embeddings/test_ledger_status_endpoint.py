...
@pytest.mark.unit
def test_ledger_status_happy_path(disable_heavy_startup, admin_user, fake_redis):
    ...
    fake_redis._kv[f"embeddings:ledger:idemp:{idk}"] = json.dumps({"status": "completed", "ts": 123, "job_id": "job-x"})
    fake_redis._kv[f"embeddings:ledger:dedupe:{ddk}"] = json.dumps({"status": "in_progress", "ts": 456})
...
@pytest.mark.unit
def test_ledger_status_requires_key_and_admin(disable_heavy_startup, fake_redis, monkeypatch):
    client = TestClient(app)
    client.cookies.set("csrf_token", "x")
    client.headers["X-CSRF-Token"] = "x"
    client.headers["Authorization"] = "Bearer key"
