...
@pytest.mark.unit
def test_orchestrator_sse_first_event_direct(fake_redis):
    async def _take_first():
        agen = _sse_orchestrator_stream(fake_redis)
        first = await agen.__anext__()
        return first
    data_line = asyncio.run(_take_first())
...
def test_orchestrator_sse_flags_reflected(fake_redis, stage):
    ...
    data_line = asyncio.run(_take_first_after_flags())
    lines = [l for l in data_line.splitlines() if l.strip()]
    data_lines = [l for l in lines if l.startswith('data: ')]
    assert data_lines, f"No data line in SSE chunk: {data_line!r}"
    payload = json.loads(data_lines[0][6:])
